...
impl WalletManager {
    ...
    pub fn load_wallet(&self, name: &str) -> Result<Wallet> {
        ...
        let contents = fs::read_to_string(&path).context("Failed to read wallet file")?;

        let wallet: Wallet =
            serde_json::from_str(&contents).context("Failed to parse wallet file")?;
    ...
    fn save_wallet(&self, wallet: &Wallet) -> Result<()> {
        ...
        let contents =
            serde_json::to_string_pretty(wallet).context("Failed to serialize wallet")?;

        fs::write(&path, contents).context("Failed to write wallet file")?;
