...
pub async fn send(config: &Config, to: String, amount: f64) -> Result<()> {
    ...
    println!(
        "Estimated Fee: {} NOVA",
        format!("{:.8}", estimated_fee).yellow()
    );
    println!(
        "Total: {} NOVA",
        format!("{:.8}", amount + estimated_fee).yellow().bold()
    );
        ...
        Ok(txid) => {
            ...
            match &config.output_format {
                ...
                _ => {
                    ...
                    println!(
                        "Track at: https://explorer.testnet.supernovanetwork.xyz/tx/{}",
                        txid
                    );
                }
            }
...
pub async fn get(config: &Config, txid: String) -> Result<()> {
    ...
    match client.get_transaction(&txid).await {
        Ok(tx) => match &config.output_format {
            OutputFormat::Json => {
            ...
            if mock_history.is_empty() {
                ...
                for tx in mock_history {
                    ...
                    let time = chrono::Local
                        .timestamp_opt(tx_obj["time"].as_i64().unwrap(), 0)
                        .unwrap();
