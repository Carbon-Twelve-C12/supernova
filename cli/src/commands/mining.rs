...
pub async fn status(config: &Config) -> Result<()> {
    ...
    match client.get_mining_info().await {
        Ok(info) => match &config.output_format {
            OutputFormat::Json => {
...
pub async fn start(config: &Config, threads: Option<u32>) -> Result<()> {
    ...
    let thread_count = if let Some(t) = threads {
        ...
        let input: String = Input::new()
            .with_prompt(format!(
                "Number of mining threads (1-{}, default: {})",
                cpu_count, default_threads
            ))
    ...
    println!(
        "Network Hashrate: {}",
        format_hashrate(current_info.network_hashrate)
    );
...
pub async fn benchmark(config: &Config) -> Result<()> {
    ...
    let mock_results: Vec<(String, u64)> = vec![
        ...
        (
            format!("{} threads", cpu_count / 2),
            145_000 * (cpu_count as u64 / 2) * 85 / 100,
        ),
        (
            format!("{} threads", cpu_count),
            145_000 * cpu_count as u64 * 75 / 100,
        ),
    ];

    match &config.output_format {
        OutputFormat::Json => {
            let results: Vec<serde_json::Value> = mock_results
                .iter()
        ...
        _ => {
            ...
            for (config_name, hashrate) in &mock_results {
                ...
                table.add_row(Row::new(vec![
                    ...
                    Cell::new(&format!("{}%", efficiency)).style_spec(if efficiency > 80 {
                        "Fg"
