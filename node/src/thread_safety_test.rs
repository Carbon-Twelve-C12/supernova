...
mod thread_safety_tests {
    ...
    use super::super::thread_safety_fix::{NodeApiFacade, ThreadSafeNode};
    use std::sync::Arc;
    ...
    fn test_node_can_be_shared_across_threads() {
        ...
        let node = Arc::new(
            rt.block_on(Node::new(NodeConfig::default()))
                .expect("node init"),
        );
    ...
    async fn test_concurrent_node_access() {
        ...
        for i in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                ...
                match i % 4 {
                    0 => {
                        let _ = facade_clone.get_info();
                    }
                    1 => {
                        let _ = facade_clone.get_status().await;
                    }
                    2 => {
                        let _ = facade_clone.get_performance_metrics();
                    }
                    _ => {
                        let _ = facade_clone.get_system_info();
                    }
                }
            });
