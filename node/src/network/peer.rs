...
impl PeerManager {
    ...
    pub fn update_peer_reputation(&self, peer_id: &PeerId, delta: i32) {
        if let Some(mut peer) = self.peers.get_mut(peer_id) {
            ...
            if peer.reputation <= -80 {
                self.ban_peer(peer_id, self.default_ban_duration, BanReason::Misbehavior);
            }
        }
    }
    ...
    pub fn ban_peer(&self, peer_id: &PeerId, duration: Duration, reason: BanReason) {
        ...
        if let Some(mut peer) = self.peers.get_mut(peer_id) {
            ...
            if let Some(network_info) = &peer.network_info {
                if matches!(
                    ...
                    self.banned_subnets
                        .insert(network_info.subnet.clone(), (Instant::now(), duration));

                    warn!(
                        "Banned subnet {:?} due to peer {} misbehavior: {:?}",
        ...
        self.banned_peers
            .insert(*peer_id, (expiration, duration, reason.clone()));
    ...
    pub fn inbound_connection_count(&self) -> usize {
        self.peers
            .iter()
            .filter(|p| {
                p.is_inbound && (p.state == PeerState::Connected || p.state == PeerState::Ready)
            })
            .count()
    }
    ...
    fn prune_old_peers(&self) {
        ...
        let old_peers: Vec<_> = self
            .peers
            .iter()
            .filter(|p| p.last_seen < one_week_ago && p.state == PeerState::Disconnected)
            .map(|p| p.peer_id)
            .collect();
...
mod tests {
    ...
    fn test_peer_management() {
        ...
        manager.ban_peer(
            ...
            BanReason::Manual("Test ban".to_string()),
        );
