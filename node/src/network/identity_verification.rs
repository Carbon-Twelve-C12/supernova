...
impl IdentityVerificationSystem {
    ...
    pub async fn create_challenge(&self, peer_id: &PeerId) -> IdentityChallenge {
        ...
        // Store the challenge
        self.challenges.write().await.insert(*peer_id, challenge.clone());
        self.verification_status.write().await.insert(*peer_id, VerificationStatus::Challenged);
        
        info!("Created identity challenge for peer {} with difficulty {}", peer_id, self.challenge_difficulty);
        challenge
    }
    ...
    pub async fn verify_challenge(&self, peer_id: &PeerId, solution: &[u8]) -> bool {
        ...
        if challenge.is_expired() {
            ...
            self.verification_status.write().await.insert(*peer_id, VerificationStatus::Failed);
            return false;
        }
        ...
        if is_valid {
            ...
            self.challenges.write().await.remove(peer_id);
            self.verification_status.write().await.insert(*peer_id, VerificationStatus::Verified);
            self.verification_times.write().await.insert(*peer_id, Instant::now());
    ...
    pub async fn get_status(&self, peer_id: &PeerId) -> VerificationStatus {
        self.verification_status.read().await
            .get(peer_id)
            .copied()
    ...
    pub async fn cleanup_expired(&self) {
        ...
        let expired_challenges: Vec<PeerId> = challenges.iter()
        ...
        let expired_verifications: Vec<PeerId> = times.iter()
...
mod tests {
    ...
    async fn test_verification_system() {
        ...
        assert_eq!(system.get_status(&peer_id).await, VerificationStatus::Challenged);
        ...
        assert_eq!(system.get_status(&peer_id).await, VerificationStatus::Verified);
    }
} 