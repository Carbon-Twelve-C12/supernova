use crate::{network::peer::PeerInfo, storage::Storage};
use libp2p::PeerId;
use std::{
...
impl PeerManager {
    ...
    pub async fn add_peer(
        ...
        let available_slots =
            self.connection_limits.max_peers - self.connection_limits.reserved_slots;
    ...
    pub async fn update_peer_info(
        &self,
        peer_id: &PeerId,
        update_fn: impl FnOnce(&mut PeerInfo),
    ) -> Result<(), Box<dyn std::error::Error>> {
        let mut peers = self.peers.write().await;
    ...
    pub async fn get_peers_to_disconnect(&self, count: usize) -> Vec<PeerId> {
        ...
        let mut peer_scores: Vec<(PeerId, f64)> = scores
            .iter()
