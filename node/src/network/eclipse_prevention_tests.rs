...
mod eclipse_prevention_tests {
    ...
    async fn test_subnet_diversity_enforcement() {
        ...
        for i in 0..5 {
            ...
            assert!(system.register_connection(peer_id, ip, true, false).await.is_ok());
        }
        ...
        let peer_id = PeerId::random();
        assert!(system.should_allow_connection(&peer_id, subnet_ip_base, true).await.is_ok());
        system.register_connection(peer_id, subnet_ip_base, true, false).await.unwrap();
        ...
        assert!(result.unwrap_err().contains("Too many connections from subnet"));
    }
    ...
    async fn test_asn_diversity_enforcement() {
        ...
        for i in 0..4 {
            ...
            system.register_connection(peer_id, ip, true, false).await.unwrap();
        }
        ...
        assert!(system.should_allow_connection(&peer_id, ip, true).await.is_ok());
    }
    ...
    async fn test_inbound_outbound_ratio() {
        ...
        system.register_connection(peer_id, ip, false, false).await.unwrap();
    ...
    async fn test_peer_rotation() {
        ...
        for i in 8..10 {
            ...
            system.register_connection(peer_id, ip, false, true).await.unwrap();
        }
    ...
    async fn test_behavioral_scoring() {
        ...
        let peer_id = PeerId::random();
        let ip = IpAddr::V4(Ipv4Addr::new(192, 168, 1, 1));
        system.register_connection(peer_id.clone(), ip, true, false).await.unwrap();
    ...
    async fn test_eclipse_attack_detection() {
        ...
        for i in 0..8 {
            ...
            system.record_peer_advertisement(from_peer, vec![advertised_peer.clone()]).await;
        }
    ...
    async fn test_concurrent_connection_handling() {
        ...
        for i in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                let peer_id = PeerId::random();
                let ip = IpAddr::V4(Ipv4Addr::new(192, 168, 1, i + 1));
                system_clone.should_allow_connection(&peer_id, ip, true).await
            });
            handles.push(handle);
        }
        ...
        assert!(successes <= 1, "Too many connections from same subnet allowed: {}", successes);
    }
