...
pub enum ProxyRequest {
    ...
    GetPeer(String, oneshot::Sender<Result<Option<PeerInfo>, Box<dyn Error + Send>>>),
    AddPeer(String, bool, oneshot::Sender<Result<PeerAddResponse, Box<dyn Error + Send>>>),
    RemovePeer(String, oneshot::Sender<Result<bool, Box<dyn Error + Send>>>),
    GetBandwidthUsage(u64, oneshot::Sender<Result<BandwidthUsage, Box<dyn Error + Send>>>),
...
impl NetworkProxy {
    ...
    pub fn new(
        ...
        network_id: String,
        command_tx: mpsc::Sender<NetworkCommand>,
    ) -> (Self, mpsc::Receiver<ProxyRequest>, Arc<RwLock<NetworkStats>>) {
            ...
            Ok(Some(request)) => {
                match request {
                    ProxyRequest::GetNetworkInfo(tx) => {
                        let result = p2p_network.get_network_info().await
                            .map_err(|e| -> Box<dyn Error + Send> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())) });
                        let _ = tx.send(result);
                    }
                    ProxyRequest::GetConnectionCount(tx) => {
                        let result = p2p_network.get_connection_count().await
                            .map_err(|e| -> Box<dyn Error + Send> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())) });
                        let _ = tx.send(result);
                    }
                    ProxyRequest::GetPeers(tx) => {
                        let result = p2p_network.get_peers().await
                            .map_err(|e| -> Box<dyn Error + Send> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())) });
                        let _ = tx.send(result);
                    }
                    ProxyRequest::GetPeer(peer_id, tx) => {
                        let result = p2p_network.get_peer(&peer_id).await
                            .map_err(|e| -> Box<dyn Error + Send> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())) });
                        let _ = tx.send(result);
                    }
                    ProxyRequest::AddPeer(address, permanent, tx) => {
                        let result = p2p_network.add_peer(&address, permanent).await
                            .map_err(|e| -> Box<dyn Error + Send> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())) });
                        let _ = tx.send(result);
                    }
                    ProxyRequest::RemovePeer(peer_id, tx) => {
                        let result = p2p_network.remove_peer(&peer_id).await
                            .map_err(|e| -> Box<dyn Error + Send> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())) });
                        let _ = tx.send(result);
                    }
                    ProxyRequest::GetBandwidthUsage(period, tx) => {
                        let result = p2p_network.get_bandwidth_usage(period).await
                            .map_err(|e| -> Box<dyn Error + Send> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())) });
                        let _ = tx.send(result);
                    }
    ...
    pub async fn get_network_info(&self) -> Result<NetworkInfo, Box<dyn Error>> {
        ...
        self.request_tx.send(ProxyRequest::GetNetworkInfo(tx)).await
            .map_err(|_| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, "Network proxy channel closed")) })?;
        rx.await.map_err(|_| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, "Network request cancelled")) })?
    ...
    pub async fn get_connection_count(&self) -> Result<ConnectionCount, Box<dyn Error>> {
        let (tx, rx) = oneshot::channel();
        self.request_tx.send(ProxyRequest::GetConnectionCount(tx)).await
            .map_err(|_| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, "Network proxy channel closed")) })?;
        rx.await.map_err(|_| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, "Network request cancelled")) })?
            .map_err(|e| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())) })
    }
    ...
    pub async fn get_peers(&self) -> Result<Vec<PeerInfo>, Box<dyn Error>> {
        let (tx, rx) = oneshot::channel();
        self.request_tx.send(ProxyRequest::GetPeers(tx)).await
            .map_err(|_| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, "Network proxy channel closed")) })?;
        rx.await.map_err(|_| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, "Network request cancelled")) })?
            .map_err(|e| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())) })
    }
    ...
    pub async fn get_peer(&self, peer_id: &str) -> Result<Option<PeerInfo>, Box<dyn Error>> {
        let (tx, rx) = oneshot::channel();
        self.request_tx.send(ProxyRequest::GetPeer(peer_id.to_string(), tx)).await
            .map_err(|_| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, "Network proxy channel closed")) })?;
        rx.await.map_err(|_| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, "Network request cancelled")) })?
            .map_err(|e| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())) })
    }
    ...
    pub async fn add_peer(&self, address: &str, permanent: bool) -> Result<PeerAddResponse, Box<dyn Error>> {
        let (tx, rx) = oneshot::channel();
        self.request_tx.send(ProxyRequest::AddPeer(address.to_string(), permanent, tx)).await
            .map_err(|_| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, "Network proxy channel closed")) })?;
        rx.await.map_err(|_| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, "Network request cancelled")) })?
            .map_err(|e| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())) })
    }
    ...
    pub async fn remove_peer(&self, peer_id: &str) -> Result<bool, Box<dyn Error>> {
        let (tx, rx) = oneshot::channel();
        self.request_tx.send(ProxyRequest::RemovePeer(peer_id.to_string(), tx)).await
            .map_err(|_| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, "Network proxy channel closed")) })?;
        rx.await.map_err(|_| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, "Network request cancelled")) })?
            .map_err(|e| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())) })
    }
    ...
    pub async fn get_bandwidth_usage(&self, period: u64) -> Result<BandwidthUsage, Box<dyn Error>> {
        let (tx, rx) = oneshot::channel();
        self.request_tx.send(ProxyRequest::GetBandwidthUsage(period, tx)).await
            .map_err(|_| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, "Network proxy channel closed")) })?;
        rx.await.map_err(|_| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, "Network request cancelled")) })?
            .map_err(|e| -> Box<dyn Error> { Box::new(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())) })
    }
    ...
    pub fn peer_count_sync(&self) -> usize {
        ...
        tokio::task::block_in_place(|| {
            ...
            rt.block_on(async {
                self.cached_stats.read().await.peers_connected
            })
        })
    }
    ...
    pub fn broadcast_transaction(&self, tx: &btclib::types::transaction::Transaction) {
        ...
        tokio::spawn(async move {
            let _ = command_tx.send(NetworkCommand::AnnounceTransaction {
                transaction: tx,
    ...
    pub fn broadcast_block(&self, block: &btclib::types::block::Block) {
        ...
        // Fire and forget
        tokio::spawn(async move {
            let _ = command_tx.send(NetworkCommand::AnnounceBlock {
                block: block.clone(),
                height: block.height(),
