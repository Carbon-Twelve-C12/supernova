...
pub trait SyncMetrics: Send + Sync {
    ...
    async fn record_fork_detection(
        &self,
        old_tip: [u8; 32],
        new_tip: [u8; 32],
        fork_height: u64,
        blocks_disconnected: u64,
        blocks_connected: u64,
        reason: String,
    );
}
...
impl SyncMetrics for DefaultSyncMetrics {
    ...
    async fn record_sync_completed(&self, final_height: u64, duration_secs: u64) {
        info!(
            "Sync completed at height {}. Duration: {} seconds",
            final_height, duration_secs
        );
    }
...
impl PeerData {
    ...
    fn record_response_time(&mut self, response_time_ms: u64) {
        ...
        if self.response_time_samples == 0 {
            ...
            self.avg_response_time = (self.avg_response_time * self.response_time_samples
                + response_time_ms)
    ...
    fn check_for_uptime_bonus(&mut self) {
        ...
        if uptime.as_secs() > 3600 && // 1 hour
           self.last_scoring_adjustment.elapsed().as_secs() > 3600
        {
            // Don't give bonus more than once per hour
...
impl ChainSync {
    ...
    pub async fn load_checkpoints(&mut self) -> Result<(), StorageError> {
        ...
        if let Some(checkpoint_data) = self.db.get_metadata(b"checkpoints")? {
            let checkpoints: Vec<Checkpoint> =
                bincode::deserialize(&checkpoint_data).map_err(StorageError::Serialization)?;
    ...
    pub fn update_peer_height(&self, peer_id: &PeerId, height: u64, total_difficulty: u64) {
        if let Some(mut peer) = self.peer_data.get_mut(peer_id) {
            ...
            if height > self.highest_seen_height
                || (height == self.highest_seen_height
                    && total_difficulty > self.highest_seen_total_difficulty)
            {
                debug!(
                    "New highest chain detected: height={}, difficulty={}",
                    height, total_difficulty
                );
            }
        }
    }
    ...
    pub async fn handle_new_block(
        ...
        } else if height == self.highest_seen_height
            && total_difficulty > self.highest_seen_total_difficulty
        {
            self.highest_seen_total_difficulty = total_difficulty;
        }
        ...
        match &mut self.sync_state {
            SyncState::Idle => {
                ...
                if height > self.chain_state.get_height() + 10 {
                    ...
                    self.request_missing_blocks(self.chain_state.get_height() + 1, height)
                        .await?;
                }
            }
            SyncState::SyncingBlocks {
                ...
                if blocks_requested.contains(&block_hash) {
                    ...
                    if blocks_requested.is_empty()
                        || blocks_received.len() >= MAX_BLOCKS_PER_REQUEST as usize
                    {
            ...
            SyncState::VerifyingBlocks { .. } => {
                ...
                let block_hash = block.hash();
                debug!(
                    "Received block {} while verifying, saving for later",
    ...
    pub async fn handle_block_headers(
        ...
        if let Some(peer_id) = from_peer {
            ...
            if let Some(mut peer_data) = self.peer_data.get_mut(&peer_id) {
                ...
                self.record_peer_response_time(&peer_id, response_time)
                    .await;

                // Record headers provided by this peer
                self.record_peer_headers(&peer_id, headers_count as u64, headers_valid)
                    .await;
        ...
        if let Some(peer_id) = from_peer {
            ...
            match &self.sync_state {
                ...
                _ => {
                    ...
                    debug!(
                        "Received {} headers while not in header sync mode",
                        headers_count
                    );
                }
            }
    ...
    pub async fn handle_blocks(
                ...
                for block in blocks {
                    ...
                    match self.chain_state.process_block(block.clone()).await {
                        Ok(true) => {
                            ...
                            self.metrics
                                .record_block_validation(true, duration_ms)
                                .await;
                        ...
                        Err(e) => {
                            ...
                            let duration_ms = process_start.elapsed().as_millis() as u64;
                            self.metrics
                                .record_block_validation(false, duration_ms)
                                .await;
        ...
        self.metrics
            .record_sync_progress(current_height, highest_peer_height)
            .await;
    ...
    pub async fn start_sync(
        ...
        info!(
            "Starting blockchain sync from height {} to {}",
            current_height, target_height
        );
        self.metrics.record_sync_started(target_height).await;

        // Start with header synchronization
        self.start_header_sync(current_height, target_height)
            .await?;
    ...
    async fn start_header_sync(
        ...
        let actual_end = std::cmp::min(actual_start + MAX_HEADERS_PER_REQUEST, end_height);
        ...
        self.request_headers(actual_start, actual_end, best_peer)
            .await?;

        info!(
            "Requesting headers from height {} to {}",
    ...
    async fn process_headers(
        ...
        if next_height >= end_height || headers.len() < MAX_HEADERS_PER_REQUEST as usize {
            ...
            self.start_header_sync(next_height, self.highest_seen_height)
                .await?;
        }
    ...
    async fn start_block_verification(&mut self) -> Result<(), String> {
        // Ensure we're in the block syncing state
        let (headers, mut blocks_received) =
            match std::mem::replace(&mut self.sync_state, SyncState::Idle) {
                SyncState::SyncingBlocks {
                    headers,
                    blocks_received,
                    ..
        ...
        if blocks_to_verify.is_empty() {
            ...
            self.start_header_sync(self.chain_state.get_height(), self.highest_seen_height)
                .await?;
    ...
    async fn verify_next_block(&mut self) -> Result<(), String> {
        loop {
                    ...
                    if let Some(block) = blocks.pop_front() {
                        ...
                        if self.chain_state.get_height() < self.highest_seen_height {
                            ...
                            if let Some(start_time) = self.sync_start_time {
                                ...
                                self.metrics
                                    .record_sync_completed(self.chain_state.get_height(), duration)
                                    .await;
            ...
            self.metrics
                .record_block_validation(verification_result.is_ok(), duration_ms)
                .await;

            // Update verification start time for next block
            if let SyncState::VerifyingBlocks {
                current_verification_start,
                ..
            } = &mut self.sync_state
            {
    ...
    async fn process_single_block(&mut self, block: Block) -> Result<(), String> {
        ...
        match self.chain_state.process_block(block.clone()).await {
            Ok(true) => {
                ...
                self.metrics
                    .record_block_validation(true, validation_time)
                    .await;
            ...
            Err(e) => {
                ...
                let validation_time = process_start.elapsed().as_millis() as u64;
                self.metrics
                    .record_block_validation(false, validation_time)
                    .await;
    ...
    fn find_best_checkpoint_height(&self, current_height: u64) -> u64 {
        ...
        for checkpoint in &self.checkpoints {
            if checkpoint.height > current_height
                && (best_checkpoint == 0 || checkpoint.height < best_checkpoint)
            {
    ...
    async fn request_missing_blocks(
        ...
        info!(
            "Requesting missing blocks from {} to {}",
            start_height, end_height
        );
    ...
    fn save_block_for_later(&self, block: Block, height: u64) -> Result<(), String> {
        ...
        debug!(
            "Saved block {} at height {} for later processing",
            hex::encode(&block_hash[..4]),
            height
        );
    ...
    fn get_peers_for_block_requests(&self, count: usize) -> Vec<PeerId> {
        ...
        if peers.len() < count {
            let mut additional_candidates: Vec<(PeerId, i32, bool)> = self
                .peer_data
                .iter()
                .filter(|entry| !peers.contains(entry.key()) && !entry.value().is_on_probation)
                .map(|entry| {
                    let peer_data = entry.value();
                    // Calculate an effective score that rewards reliable peers
                    let effective_score =
                        peer_data.score + (if peer_data.is_reliable() { 20 } else { 0 });
    ...
    async fn penalize_peer(&self, peer_id: &PeerId, penalty: i32) {
        if let Some(mut peer_data) = self.peer_data.get_mut(peer_id) {
            ...
            debug!(
                "Penalized peer {} with {} points, new score: {}",
            ...
            if peer_data.should_disconnect() {
                warn!(
                    "Disconnecting peer {} due to low score or excessive timeouts",
                    peer_id
                );
    ...
    async fn reward_peer(&self, peer_id: &PeerId, points: i32) {
        if let Some(mut peer_data) = self.peer_data.get_mut(peer_id) {
            ...
            debug!(
                "Rewarded peer {} with {} points, new score: {}",
    ...
    fn get_preferred_peers(&self, min_count: usize) -> Vec<PeerId> {
        let mut preferred_peers: Vec<PeerId> = self
            .peer_data
            .iter()
        ...
        if preferred_peers.len() < min_count {
            let mut additional_peers: Vec<(PeerId, i32)> = self
                .peer_data
                .iter()
                .filter(|entry| !entry.value().is_preferred)
    ...
    async fn update_sync_progress(&mut self) {
        ...
        if current_height < target_height {
            self.metrics
                .record_sync_progress(current_height, target_height)
                .await;
        }
    }
    ...
    async fn start_block_download(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        for peer_id in peers {
            ...
            let block_hashes: Vec<[u8; 32]> = peer_headers.iter().map(|h| h.hash()).collect();
    ...
    async fn handle_reorganization_event(&mut self, event: &ReorganizationEvent) {
        ...
        match &mut self.sync_state {
            SyncState::SyncingBlocks { .. } | SyncState::VerifyingBlocks { .. } => {
            ...
            SyncState::Idle => {
                ...
                if event.blocks_disconnected > 2 {
                    debug!(
                        "Significant reorganization detected, checking if further sync is needed"
                    );
                    let current_height = self.chain_state.get_height();
    ...
    fn test_header_validation() {
        ...
        assert_eq!(
            *header.prev_block_hash(),
            hash1,
            "Header should store prev_block_hash correctly"
        );
    }
