...
pub use method_adapters::{
    BlockNodeMethods, ChainStateNodeMethods, CloneableReadGuard, DashMapRefExt, HashMapNodeExt,
    ResultNodeMethods, TransactionPoolNodeMethods,
};

pub use trait_implementations::{
    deserialize_peer_id, safe_numeric_cast, serialize_peer_id, BorrowHelper, IVecConversion,
    ResultConversionExt, SafeNumericConversion, SerializableFuture, SerializablePeerId,
...
pub mod result_adapters {
        ...
        E: std::fmt::Display,
    {
        match result {
            ...
            Err(e) => {
                warn!(
                    "Error converting u32 to u64: {}, using default {}",
                    e, default
                );
                default
            }
...
pub mod hash_adapters {
    ...
    pub fn hex_to_hash(hex: &str) -> Result<[u8; 32], String> {
        ...
        let mut hash = [0u8; 32];
        hex::decode_to_slice(hex, &mut hash).map_err(|e| format!("Invalid hex: {}", e))?;
        Ok(hash)
    }
...
pub mod result_converters {
    ...
    pub fn convert_result_to_u64(
        result: Result<u32, ChainStateError>,
    ...
    pub fn convert_error<T>(
        result: Result<T, ChainStateError>,
        context: &str,
    ) -> Result<T, String> {
        result.map_err(|e| {
            error!("{}: {}", context, e);
...
pub mod error_adapters {
    ...
    impl From<ChainStateError> for StorageError {
        fn from(err: ChainStateError) -> Self {
            match err {
                ChainStateError::BlockNotFound(hash) => {
                    StorageError::KeyNotFound(format!("Block not found: {}", hash))
                }
                ChainStateError::BlockAlreadyExists(hash) => {
                    StorageError::DatabaseError(format!("Block already exists: {}", hash))
                }
                ...
                ChainStateError::UtxoError(msg) => {
                    StorageError::DatabaseError(format!("UTXO error: {}", msg))
                }
