...
impl IntegrityRepairer {
    ...
    pub async fn repair_issues(&self, issues: &[IntegrityIssue]) -> Result<usize, StorageError> {
        ...
        for issue in issues {
            ...
            match issue.issue_type {
                ...
                IssueType::ChainInconsistency | IssueType::MissingReference => {
        ...
        if !structure_issues.is_empty() {
            info!(
                "Repairing {} database structure issues",
                structure_issues.len()
            );
    ...
    async fn repair_structure_issues(
        &self,
        ...
        for issue in issues {
            ...
            debug!(
                "Repairing structure issue: {} using strategy {:?}",
    ...
    async fn repair_chain_issues(&self, issues: &[&IntegrityIssue]) -> Result<usize, StorageError> {
        ...
        let checkpoint_issues: Vec<_> = issues
            .iter()
        ...
        if !checkpoint_issues.is_empty() {
            ...
            if checkpoint_height > 0 {
                ...
                let strategy = RecoveryStrategy::RevertToCheckpoint { checkpoint_height };
                ...
                match Self::execute_repair_strategy(&strategy, &corruption_type, &mut handler).await
                {
                    Ok(true) => {
                        ...
                        info!(
                            "Successfully reverted to checkpoint at height {}",
                            checkpoint_height
                        );
                    }
                    Ok(false) => {
                        warn!(
                            "Failed to revert to checkpoint at height {}",
                            checkpoint_height
                        );
                    }
                    Err(e) => {
        ...
        let missing_block_issues: Vec<_> = issues
            .iter()
            ...
            if let IssueLocation::Block { hash, .. } = issue.location {
                ...
                match Self::execute_repair_strategy(&strategy, &corruption_type, &mut handler).await
                {
                    Ok(true) => {
                        repaired += 1;
                        info!(
                            "Successfully repaired missing block {}",
                            hex::encode(&hash[..8])
                        );
                    }
                    Ok(false) => {
    ...
    async fn repair_utxo_issues(&self, issues: &[&IntegrityIssue]) -> Result<usize, StorageError> {
        ...
        let critical_utxo_issues = issues
            .iter()
        ...
        match &issue.issue_type {
            ...
            IssueType::ChainInconsistency => {
                let affected_range = match &issue.location {
                    ...
                    IssueLocation::Block { height, .. } => height.map(|h| (h, h)),
                    _ => None,
                };
            ...
            IssueType::MissingReference => match &issue.location {
                IssueLocation::Block { hash, .. } => CorruptionType::RecordCorruption {
                    tree_name: "blocks".to_string(),
                    affected_keys: vec![hash.to_vec()],
                },
                IssueLocation::Transaction { hash, .. } => CorruptionType::RecordCorruption {
    ...
    async fn execute_repair_strategy(
        ...
        match strategy {
            RecoveryStrategy::RestoreFromBackup => match handler.repair_from_backup().await {
            ...
            RecoveryStrategy::RebuildChainState {
                ...
                match handler
                    .rebuild_chain_state(*start_height, *end_height)
                    .await
                {
                    Ok((success, _, _, _)) => Ok(success),
                    Err(e) => Err(map_corruption_error(e)),
