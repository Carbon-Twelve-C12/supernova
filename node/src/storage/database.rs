...
impl BlockchainDB {
    ...
    pub fn with_config<P: AsRef<Path>>(
        ...
        let (block_cache, tx_cache, header_cache, utxo_cache) = if db_config.use_lru_cache {
            (
                ...
                Some(Arc::new(DatabaseCache::new(
                    db_config.header_cache_capacity,
                ))),
    ...
    fn init_bloom_filters(&mut self) -> Result<(), StorageError> {
        ...
        for result in self.blocks.iter() {
            let (key, _) = result?;
            let mut block_filter = self.block_filter.write().map_err(|e| {
                StorageError::LockPoisoned(format!("Block filter write lock poisoned: {}", e))
            })?;
        ...
        for result in self.transactions.iter() {
            let (key, _) = result?;
            let mut tx_filter = self.tx_filter.write().map_err(|e| {
                StorageError::LockPoisoned(format!("Transaction filter write lock poisoned: {}", e))
            })?;
    ...
    pub fn get_block(&self, block_hash: &[u8; 32]) -> Result<Option<Block>, StorageError> {
        ...
        if self.config.use_bloom_filters {
            let block_filter = self.block_filter.read().map_err(|e| {
                StorageError::LockPoisoned(format!("Block filter read lock poisoned: {}", e))
            })?;
    ...
    pub fn store_pending_block(
        ...
        // Create metadata
        let mut metadata = PendingBlockMetadata::new(*block_hash, height, source);
    ...
    pub fn get_pending_block_metadata(
        &self,
        block_hash: &[u8; 32],
    ) -> Result<Option<PendingBlockMetadata>, StorageError> {
        if let Some(meta_data) = self.pending_blocks_meta.get(block_hash)? {
            let metadata: PendingBlockMetadata = bincode::deserialize(&meta_data)?;
            Ok(Some(metadata))
        } else {
    ...
    pub fn update_pending_block_metadata(
        &self,
        metadata: &PendingBlockMetadata,
    ) -> Result<(), StorageError> {
        let meta_data = bincode::serialize(metadata)?;
    ...
    pub fn get_transaction(&self, tx_hash: &[u8; 32]) -> Result<Option<Transaction>, StorageError> {
        ...
        // Check bloom filter for fast negative lookups
        if self.config.use_bloom_filters {
            let tx_filter = self.tx_filter.read().map_err(|e| {
                StorageError::LockPoisoned(format!("Transaction filter read lock poisoned: {}", e))
            })?;
    ...
    pub fn store_block_height_index(
        &self,
        height: u64,
        block_hash: &[u8; 32],
    ) -> Result<(), StorageError> {
    ...
    pub fn list_trees(&self) -> Result<Vec<String>, StorageError> {
        ...
        let tree_names = match self.get_metadata(b"tree_registry") {
            Ok(Some(data)) => {
                bincode::deserialize::<Vec<String>>(&data).map_err(StorageError::Serialization)?
            }
            _ => {
    ...
    pub fn open_tree(&self, name: &str) -> Result<sled::Tree, StorageError> {
        let tree = self.db.open_tree(name).map_err(StorageError::Database)?;
    ...
    fn register_tree(&self, name: &str) -> Result<(), StorageError> {
        ...
        if !trees.contains(&name.to_string()) {
            ...
            let serialized = bincode::serialize(&trees).map_err(StorageError::Serialization)?;
    ...
    pub fn tree_contains_key(&self, tree_name: &str, key: &[u8]) -> Result<bool, StorageError> {
        ...
        tree.contains_key(key).map_err(StorageError::Database)
    }
    ...
    fn is_valid_record(
        ...
        match tree_name {
            BLOCKS_TREE => match bincode::deserialize::<Block>(value) {
                Ok(block) => Ok(block.validate()),
                Err(_) => Ok(false),
            },
            TXNS_TREE => bincode::deserialize::<Transaction>(value)
                .map(|_| true)
                .or(Ok(false)),
            HEADERS_TREE => bincode::deserialize::<BlockHeader>(value)
                .map(|_| true)
                .or(Ok(false)),
            ...
            if self.metadata.get(key.as_bytes())?.is_none() {
                ...
                if repair {
                    ...
                    if *key == "height" {
                        ...
                        if max_height > 0 {
                            self.metadata
                                .insert("height".as_bytes(), &max_height.to_be_bytes())?;
                        }
                    }
                }
            }
                        ...
                        if let Some(height_data) = self.metadata.get("height".as_bytes())? {
                            if height_data.len() == 8 {
                                ...
                                for h in (0..=height).rev() {
                                    let height_key = h.to_be_bytes();
                                    if let Some(hash_data) =
                                        self.block_height_index.get(height_key)?
                                    {
                                        if hash_data.len() == 32 {
                                            // Check if this block exists
    ...
    pub fn store_integrity_check_result(
        &self,
        result: &IntegrityCheckResult,
    ) -> Result<(), StorageError> {
        let data = bincode::serialize(result)?;
        self.metadata
            .insert("latest_integrity_check".as_bytes(), data.as_slice())?;
        Ok(())
    }
    ...
    pub fn preload_critical_data(&self) -> Result<(), StorageError> {
            ...
            if self.config.use_bloom_filters {
                let mut tx_filter = self.tx_filter.write().map_err(|e| {
                    StorageError::LockPoisoned(format!(
                        "Transaction filter write lock poisoned: {}",
                        e
                    ))
                })?;
                tx_filter.insert(&key);
        ...
        let start_height = if current_height > 1000 {
            current_height - 1000
        } else {
            0
        };
    ...
    pub fn execute_optimized_batch(&self, batch: BatchOperation) -> Result<(), StorageError> {
        ...
        let mut tree_operations: std::collections::HashMap<String, Vec<BatchOp>> =
    ...
    pub async fn async_flush(&self) -> Result<(), StorageError> {
        ...
        let task = tokio::task::spawn_blocking(move || db_clone.flush());
    ...
    pub fn get_spending_transaction(
        &self,
        tx_hash: &[u8; 32],
        vout: u32,
    ) -> Result<Option<[u8; 32]>, StorageError> {
...
mod tests {
    ...
    fn test_spent_output_tracking() -> Result<(), StorageError> {
        ...
        assert_eq!(
            db.get_spending_transaction(&tx_hash, vout)?,
