...
pub enum JournalEntry {
    ...
    BlockWrite { hash: [u8; 32], data: Vec<u8> },
    /// Transaction write operation
    TransactionWrite { hash: [u8; 32], data: Vec<u8> },
    ...
    UtxoDelete { tx_hash: [u8; 32], index: u32 },
    /// Metadata write operation
    MetadataWrite { key: Vec<u8>, value: Vec<u8> },
    /// Height index update
    HeightIndexWrite { height: u64, block_hash: [u8; 32] },
    /// Batch operation marker
    BatchStart { batch_id: u64, timestamp: u64 },
    /// Batch commit marker
...
impl WalEntry {
    ...
    fn calculate_checksum(&self) -> u32 {
        let data =
            bincode::serialize(&(&self.sequence, &self.timestamp, &self.entry)).unwrap_or_default();
        crc32fast::hash(&data)
    }
...
impl WriteAheadLog {
    ...
    pub async fn commit_batch(&self, batch_id: u64) -> Result<(), WalError> {
        self.write_entry(JournalEntry::BatchCommit { batch_id })
            .await?;
    ...
    pub async fn rollback_batch(&self, batch_id: u64) -> Result<(), WalError> {
        self.write_entry(JournalEntry::BatchRollback { batch_id })
            .await?;
        *self.current_batch.write().await = None;
        Ok(())
    }
...
mod tests {
    ...
    async fn test_wal_basic_operations() {
        ...
        // Write some entries
        let seq1 = wal
            .write_entry(JournalEntry::MetadataWrite {
        ...
        let seq2 = wal
            .write_entry(JournalEntry::BlockWrite {
                hash: [0u8; 32],
                data: vec![1, 2, 3, 4],
            })
            .await
