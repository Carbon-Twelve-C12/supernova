...
impl CheckpointManager {
    ...
    pub async fn start(&mut self) -> Result<(), StorageError> {
        info!(
            "Starting checkpoint manager with interval of {} blocks or {} seconds",
    ...
    pub async fn stop(&mut self) -> Result<(), StorageError> {
        ...
        if let Some(tx) = &self.command_tx {
            if let Err(e) = tx.send(CheckpointCommand::Shutdown).await {
                error!(
                    "Failed to send shutdown command to checkpoint manager: {}",
                    e
                );
            }
        }
    ...
    pub async fn create_checkpoint(
        ...
        if let Some(tx) = &self.command_tx {
            tx.send(CheckpointCommand::Create(checkpoint_type))
                .await
                .map_err(|e| {
                    StorageError::DatabaseError(format!("Failed to send checkpoint command: {}", e))
                })?;
        } else {
    ...
    pub async fn restore_from_latest_checkpoint(&self) -> Result<(), StorageError> {
        ...
        info!(
            "Restoring from latest checkpoint at height {}",
            latest.height
        );
                    ...
                    .unwrap_or(false)
            {
                ...
                if info_file.exists() {
                    match fs::read(&info_file).await {
                        Ok(data) => match serde_json::from_slice::<CheckpointInfo>(&data) {
                            Ok(info) => {
                            ...
                            Err(e) => {
                                warn!(
                                    "Failed to parse checkpoint info from {}: {}",
                                    info_file.display(),
                                    e
                                );
                            }
    ...
    fn get_checkpoint_path(&self, height: u64) -> PathBuf {
        self.config
            .checkpoint_dir
            .join(format!("checkpoint_{}", height))
    }
    ...
    async fn restore_from_path(
        ...
        if hash != checkpoint.data_hash {
            verification.complete();
            return Err(StorageError::DatabaseError(format!(
                "Checkpoint integrity check failed for height {}",
                checkpoint.height
            )));
        }
        ...
        info!(
            "Successfully restored from checkpoint at height {}",
            checkpoint.height
        );
    ...
    async fn create_checkpoint_internal(
        ...
        info!(
            "Created checkpoint at height {} ({}): {}ms, {}MB",
            height,
            format!("{:?}", checkpoint_type).to_lowercase(),
    ...
    async fn cleanup_old_checkpoints(config: &CheckpointConfig) -> Result<(), StorageError> {
                        ...
                        if let Ok(height) = height.parse::<u64>() {
                            ...
                            if info_file.exists() {
                                ...
                                if let Ok(data) = fs::read(&info_file).await {
                                    if let Ok(info) =
                                        serde_json::from_slice::<CheckpointInfo>(&data)
                                    {
        ...
        let manual_checkpoints: Vec<_> = checkpoints
            .iter()
            .filter(|(_, _, cp_type)| {
                *cp_type == CheckpointType::Manual || *cp_type == CheckpointType::Shutdown
            })
            .collect();

        // Keep the most recent regular checkpoints up to the limit
        let regular_checkpoints: Vec<_> = checkpoints
            .iter()
        ...
        let checkpoints_to_keep: HashSet<_> = manual_checkpoints
            .iter()
        ...
        for (height, path, cp_type) in checkpoints {
            if !checkpoints_to_keep.contains(&height) {
                info!(
                    "Removing old checkpoint at height {} (type: {:?})",
                    height, cp_type
                );
                if let Err(e) = fs::remove_dir_all(&path).await {
                    warn!(
                        "Failed to remove old checkpoint at {}: {}",
                        path.display(),
                        e
                    );
                }
    ...
    async fn verify_checkpoint_integrity(
                            ...
                            match serde_json::from_slice::<CheckpointInfo>(&data) {
                                Ok(info) => {
                                    ...
                                    if hash == info.data_hash {
                                        ...
                                        failed += 1;
                                        warn!(
                                            "Checkpoint at height {} failed integrity check",
                                            info.height
                                        );
                                        ...
                                        let new_path =
                                            failed_dir.join(format!("checkpoint_{}", info.height));
        ...
        info!(
            "Checkpoint integrity check complete: {} verified, {} failed",
            verified, failed
        );
