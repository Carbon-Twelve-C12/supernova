...
impl BackupManager {
    ...
    pub async fn verify_existing_backups(&self) -> Result<(), StorageError> {
        ...
        info!(
            "Backup verification complete: {} succeeded, {} failed",
    ...
    pub async fn create_backup(&self) -> Result<PathBuf, StorageError> {
        ...
        let mut src_entries = fs::read_dir(db_path).await.map_err(StorageError::Io)?;
        ...
        while let Some(entry) = src_entries.next_entry().await.map_err(StorageError::Io)? {
            let src_path = entry.path();
            let file_name = src_path
                .file_name()
                .ok_or_else(|| StorageError::DatabaseError("Invalid file name".to_string()))?;
    ...
    async fn verify_backup(&self, backup_path: &Path) -> Result<bool, StorageError> {
        ...
        let mut entries = fs::read_dir(backup_path).await.map_err(StorageError::Io)?;
...
impl RecoveryManager {
    ...
    async fn verify_checkpoint(
        ...
        let block = self
            .db
            .get_block(&checkpoint.block_hash)?
    ...
    pub async fn verify_database_integrity(&self) -> Result<bool, StorageError> {
        ...
        let (blockchain_valid, utxo_valid) =
            tokio::join!(self.verify_blockchain_parallel(), self.verify_utxo_set());
    ...
    async fn verify_blockchain_parallel(&self) -> Result<bool, StorageError> {
        ...
        for result in join_all(tasks).await {
            let verified = result
                .map_err(|e| StorageError::DatabaseError(format!("Task join error: {}", e)))??; // Unwrap the inner Result<bool, StorageError>
            if !verified {
                return Ok(false);
            }
        }
    ...
    async fn verify_utxo_set(&self) -> Result<bool, StorageError> {
        ...
        while height > 0 {
            let block = self.db.get_block(&current_hash)?.ok_or_else(|| {
                StorageError::KeyNotFound(format!("Block not found: {:?}", current_hash))
            })?;
            ...
            for tx in block.transactions() {
                ...
                let tx_hash = tx.hash();
                for (index, output) in tx.outputs().iter().enumerate() {
                    verification_utxos.insert((tx_hash, index as u32), bincode::serialize(output)?);
                }
            }
    ...
    async fn perform_recovery(&mut self) -> Result<(), StorageError> {
        ...
        info!(
            "Starting recovery from checkpoint at height {}",
            checkpoint_to_recover.height
        );
    ...
    async fn rebuild_utxo_set(&mut self) -> Result<(), StorageError> {
        ...
        while height > 0 {
            ...
            for tx in block.transactions() {
                ...
                for (index, output) in tx.outputs().iter().enumerate() {
                    self.db
                        .store_utxo(&tx_hash, index as u32, &bincode::serialize(output)?)?;
                }
            }
    ...
    async fn recover_from_checkpoint(
        ...
        if current_height > checkpoint.height {
            info!(
                "Current height {} is greater than checkpoint height {}, rolling back",
            ...
            info!(
                "Current height {} is less than checkpoint height {}, rebuilding",
                current_height, checkpoint.height
            );
            ...
            if self.chain_state.get_best_block_hash() != checkpoint.block_hash {
                info!(
                    "Block hash mismatch at height {}, rebuilding",
                    checkpoint.height
                );
                self.rebuild_from_checkpoint(checkpoint).await?;
            } else {
    ...
    async fn rebuild_from_checkpoint(
        ...
        info!(
            "Rebuilding state from checkpoint at height {}",
            checkpoint.height
        );
                ...
                for tx in block.transactions() {
                    ...
                    if tx.inputs().is_empty() {
                        ...
                        if !is_coinbase {
                            ...
                            warn!(
                                "Non-coinbase transaction with no inputs at height {}",
                                height
                            );
                            return Ok(false);
                        }
