...
impl crate::storage::Storage for MemoryStorage {
    async fn save_peer_info(&self, peer_id: &PeerId, info: &PeerInfo) -> Result<(), Box<dyn Error>> {
    ...
    async fn list_all_peers(&self) -> Result<Vec<(PeerId, PeerInfo)>, Box<dyn Error>> {
        Ok(self.peers.read().await.iter().map(|(k, v)| (*k, v.clone())).collect())
    }
    ...
    async fn save_banned_peer(&self, peer_id: &PeerId, reason: &str, until: std::time::Instant) -> Result<(), Box<dyn Error>> {
        self.banned_peers.write().await.insert(*peer_id, (reason.to_string(), until));
        Ok(())
    }
    ...
    async fn cleanup_expired_bans(&self) -> Result<(), Box<dyn Error>> {
        ...
        self.banned_peers.write().await.retain(|_, (_, until)| now < *until);
        Ok(())
    }
} 