...
impl AtomicUtxoSet {
    ...
    fn load(&self) -> Result<(), StorageError> {
        ...
        if self.db_path.exists() {
            ...
            if !contents.is_empty() {
                ...
                let mut utxos = self.utxos.write().map_err(|e| {
                    StorageError::LockPoisoned(format!("UTXO lock poisoned: {}", e))
                })?;
                *utxos = loaded_utxos;
    ...
    pub fn apply_transaction(&self, tx: UtxoTransaction) -> Result<(), StorageError> {
        ...
        // With locks held, we have exclusive access - no other thread can modify these UTXOs
        {
            ...
            let spent = self.spent_outputs.read().map_err(|e| {
                StorageError::LockPoisoned(format!("Spent outputs read lock poisoned: {}", e))
            })?;
    ...
    fn apply_transaction_internal(&self, tx: &UtxoTransaction) -> Result<(), StorageError> {
        ...
        {
            let mut utxos = self.utxos.write().map_err(|e| {
                StorageError::LockPoisoned(format!("UTXO write lock poisoned: {}", e))
            })?;
            let mut spent = self.spent_outputs.write().map_err(|e| {
                StorageError::LockPoisoned(format!("Spent outputs write lock poisoned: {}", e))
            })?;
    ...
    pub fn prune_spent_outputs(&self, height_limit: u64) -> Result<usize, StorageError> {
        let spent = self.spent_outputs.write().map_err(|e| {
            StorageError::LockPoisoned(format!("Spent outputs write lock poisoned: {}", e))
        })?;
...
mod tests {
    ...
    fn test_concurrent_access() {
        let temp_dir = tempdir().unwrap();
        let utxo_set = Arc::new(AtomicUtxoSet::new(temp_dir.path().join("utxo.db")).unwrap());
