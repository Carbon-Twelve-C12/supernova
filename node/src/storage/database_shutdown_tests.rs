...
mod tests {
    ...
    fn test_clean_shutdown() {
        ...
        // Create and populate database
        {
            ...
            db.store_metadata(b"last_clean_shutdown", timestamp.to_string().as_bytes()).unwrap();
            db.store_metadata(b"shutdown_in_progress", b"false").unwrap();
    ...
    fn test_crash_recovery() {
        ...
        // Simulate crash during operation
        {
            ...
            // Store initial state
            db.store_metadata(b"best_height", &initial_height.to_le_bytes()).unwrap();
            db.store_metadata(b"best_hash", &initial_hash).unwrap();
            
            // Start a simulated operation
            db.store_metadata(b"operation_in_progress", b"true").unwrap();
        ...
        // Recovery phase
        {
            ...
            db.store_metadata(b"operation_in_progress", b"false").unwrap();
        }
    }
    ...
    fn test_multiple_restarts() {
        ...
        // Second session
        {
            ...
            counter += 1;
            db.store_metadata(b"counter", &counter.to_le_bytes()).unwrap();
    ...
    fn test_shutdown_with_pending_operations() {
        ...
        db.store_pending_block(&[1u8; 32], &vec![1, 2, 3], Some(100), None, None).unwrap();
        db.store_pending_block(&[2u8; 32], &vec![4, 5, 6], Some(101), None, None).unwrap();
        ...
        // Perform shutdown
        db.store_metadata(b"pending_at_shutdown", &pending_count.to_le_bytes()).unwrap();
        db.flush().unwrap();
    ...
    fn test_emergency_shutdown() {
        ...
        
        {
            ...
            // Add critical data
            db.store_metadata(b"critical_data", b"must_survive").unwrap();
