...
pub enum CorruptionType {
    ...
    RecordCorruption { tree_name: String, affected_keys: Vec<Vec<u8>> },
    
    /// Inconsistency between indexes
    IndexCorruption { primary_tree: String, index_tree: String, mismatched_keys: Vec<Vec<u8>> },
...
pub enum RecoveryStrategy {
    ...
    RebuildCorruptedRecords { tree_name: String, keys: Vec<Vec<u8>> },
    
    /// Rebuild indexes from primary data
    RebuildIndexes { source_tree: String, target_index: String },
    
    /// Revert to a checkpoint
    RevertToCheckpoint { checkpoint_height: u64 },
    
    /// Rebuild chain state from blocks
    RebuildChainState { start_height: u64, end_height: Option<u64> },
}
...
impl CorruptionHandler {
    ...
    pub async fn create_checkpoint(&mut self, height: u64, block_hash: [u8; 32]) -> Result<(), CorruptionError> {
    ...
    fn create_metadata_snapshot(&self) -> Result<HashMap<String, Vec<u8>>, CorruptionError> {
        ...
        let metadata_keys = ["height", "best_hash", "total_difficulty", "pruned_height", "version"];
    ...
    pub async fn check_database_integrity(&mut self) -> Result<bool, CorruptionError> {
        ...
        if let Err(e) = self.check_file_integrity().await {
            ...
            self.corruption_log.push(CorruptionType::FileLevelCorruption);
            return Ok(false);
        }
        ...
        if !corrupted_records.is_empty() {
            warn!("Found {} corrupted records across trees", corrupted_records.len());
        ...
        info!("Database integrity check completed in {:.2}s", duration.as_secs_f64());
    ...
    fn check_single_tree_integrity(&self, tree: &Tree, tree_name: &str) -> Result<Vec<IVec>, CorruptionError> {
    ...
    fn validate_record(&self, tree_name: &str, _key: &[u8], value: &[u8]) -> Result<bool, CorruptionError> {
        if value.is_empty() {
            return Ok(false);
        }
    ...
    pub async fn auto_repair(&mut self) -> Result<Vec<RepairResult>, CorruptionError> {
        ...
        info!("Starting automated repair for {} corruption issues", self.corruption_log.len());
        ...
        for corruption in &self.corruption_log {
            ...
            debug!("Selected strategy {:?} for corruption {:?}", strategy, corruption);
            
            let start_time = std::time::Instant::now();
            let repair_result = match &strategy {
                RecoveryStrategy::RestoreFromBackup => {
                    self.repair_from_backup().await
                },
