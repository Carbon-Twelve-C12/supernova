...
use super::journal::{JournalEntry, WriteAheadLog};
use std::path::Path;
use std::sync::Arc;
...
impl DatabaseShutdownHandler {
    ...
    async fn verify_integrity(&self) -> Result<(), StorageError> {
        ...
        if !result.passed {
            error!(
                "Database integrity check failed: {} issues found",
                result.issues.len()
            );
            // Log critical issues
            for issue in result.issues.iter().filter(|i| i.is_critical) {
                error!(
                    "Critical issue: {} in tree {}",
                    issue.description, issue.tree
                );
            }
...
impl DatabaseStartupHandler {
    ...
    async fn verify_and_repair(&self, db: &mut BlockchainDB) -> Result<(), StorageError> {
        ...
        if !result.passed {
            warn!(
                "Found {} integrity issues, attempted repairs",
                result.issues.len()
            );
        } else {
...
mod tests {
    ...
    async fn test_clean_shutdown() {
        ...
        let shutdown_handler =
            DatabaseShutdownHandler::new(db.clone(), wal.clone(), ShutdownConfig::default());
