...
impl AtomicTransactionPool {
        ...
        for input_ref in &inputs {
            if let Some(existing_tx) = self.spent_outputs.get(input_ref) {
                ...
                self.metrics
                    .write()
                    .map_err(|e| {
                        MempoolError::LockError(format!("Failed to acquire metrics lock: {}", e))
                    })?
    ...
    pub fn replace_transaction(
        ...
        let new_inputs: Vec<InputRef> = new_transaction
            .inputs()
            .iter()
            .map(|input| InputRef {
                tx_hash: input.prev_tx_hash(),
...
mod tests {
    ...
    fn create_test_transaction(inputs: Vec<([u8; 32], u32)>, value: u64) -> Transaction {
        let tx_inputs = inputs
            .into_iter()
            .map(|(prev_hash, index)| TransactionInput::new(prev_hash, index, vec![], 0xffffffff))
            .collect();
    ...
    fn test_concurrent_double_spend_attempts() {
        ...
        let transactions: Vec<_> = (0..10)
            .map(|i| create_test_transaction(vec![([1u8; 32], 0)], 50_000_000 - i * 1000))
            .collect();
        ...
        let handles: Vec<_> = transactions
            ...
            .map(|tx| {
                ...
                thread::spawn(move || pool_clone.add_transaction(tx, 2))
            })
            .collect();
    ...
    fn test_concurrent_add_remove() {
        ...
        let transactions: Vec<_> = (0..100)
            .map(|i| create_test_transaction(vec![([i as u8; 32], i as u32)], 50_000_000))
            .collect();

        let tx_hashes: Vec<_> = transactions.iter().map(|tx| tx.hash()).collect();

        // Add transactions concurrently
        let add_handles: Vec<_> = transactions
            .into_iter()
            .enumerate()
            .map(|(i, tx)| {
                let pool_clone = Arc::clone(&pool);
                thread::spawn(move || pool_clone.add_transaction(tx, (i + 1) as u64))
            })
            .collect();
        ...
        let remove_handles: Vec<_> = tx_hashes[..50]
            .iter()
            .map(|hash| {
                let pool_clone = Arc::clone(&pool);
                let hash = *hash;
                thread::spawn(move || pool_clone.remove_transaction(&hash))
            })
            .collect();
