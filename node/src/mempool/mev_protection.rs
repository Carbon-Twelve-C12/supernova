...
impl MEVProtection {
    ...
    pub async fn submit_commitment(
        ...
        commitments.insert(commitment, CommittedTransaction {
            commitment,
            declared_fee,
    ...
    pub async fn reveal_transaction(
        ...
        let committed = commitments.remove(&commitment)
            .ok_or_else(|| MempoolError::InvalidTransaction(
                "No matching commitment found".to_string()
            ))?;
    ...
    fn calculate_commitment(&self, tx: &Transaction, nonce: &[u8]) -> MempoolResult<Hash256> {
        ...
        let tx_bytes = bincode::serialize(tx)
            .map_err(|e| MempoolError::ValidationFailed(format!("Failed to serialize transaction: {}", e)))?;
    ...
    pub async fn cleanup_expired_commitments(&self) {
        ...
        commitments.retain(|_, committed| {
            committed.commit_time.elapsed() <= max_age
        });
    }
...
mod tests {
    ...
    async fn test_commit_reveal_flow() {
        ...
        // Phase 1: Commit
        mev_protection.submit_commitment(
            tx_hash,
            1000, // declared fee
            commitment,
            None
