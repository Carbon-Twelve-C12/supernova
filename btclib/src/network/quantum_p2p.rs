...
impl QuantumP2PConfig {
    ...
    fn serialize_handshake_for_signing(&self, handshake: &QuantumHandshake) -> Result<Vec<u8>, P2PError> {
        let data = format!(
            "{}:{}:{}:{}:{}",
            handshake.version,
            hex::encode(&handshake.quantum_pubkey),
            hex::encode(&handshake.kem_pubkey),
            handshake.supported_schemes.iter()
                .map(|s| format!("{:?}", s))
                .collect::<Vec<_>>()
                .join(","),
            handshake.timestamp
...
pub struct QuantumProtocolHandler {
    ...
    message_handlers: HashMap<String, Box<dyn Fn(&[u8]) -> Result<Vec<u8>, P2PError> + Send + Sync>>,
}
...
impl QuantumProtocolHandler {
    ...
    {
        self.message_handlers.insert(msg_type.to_string(), Box::new(handler));
    }
    ...
    pub async fn handle_message(
        ...
        // Decrypt message
        let plaintext = self.config.receive_quantum_message(peer_id, message).await?;
...
mod tests {
    ...
    async fn test_quantum_messaging() {
        ...
        config1.peer_keys.write().unwrap().insert(peer_id2, QuantumPeerInfo {
        ...
        let encrypted = config1.send_quantum_message(&peer_id2, message).await.unwrap();
        
        // Receive message
        config2.peer_keys.write().unwrap().insert(peer_id1, QuantumPeerInfo {
