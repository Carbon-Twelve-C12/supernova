...
use bitcoin::{ScriptBuf, Network, Transaction as BitcoinTransaction, TxOut, Address};
...
pub fn extract_secret_from_bitcoin_tx(tx: &BitcoinTransaction) -> Result<[u8; 32], ExtractionError> {
    // Check each input
...
impl BitcoinRpcClient {
    ...
    pub async fn get_transaction(&self, txid: &str) -> Result<BitcoinTransaction, BitcoinAdapterError> {
        use bitcoin::Txid;
        use bitcoincore_rpc::RpcApi;
        
        let txid = Txid::from_str(txid)
            .map_err(|e| BitcoinAdapterError::ParseError(e.to_string()))?;
...
pub mod script_utils {
    ...
    pub fn is_htlc_script(script: &ScriptBuf) -> bool {
        let instructions: Vec<Instruction> = match script.instructions().collect::<Result<Vec<_>, _>>() {
            Ok(inst) => inst,
            Err(_) => return false,
        };
    ...
    pub fn extract_timeout_from_script(script: &ScriptBuf) -> Option<u32> {
        let instructions: Vec<Instruction> = script.instructions().collect::<Result<Vec<_>, _>>().ok()?;
