...
pub enum WsMessage {
    ...
    Unsubscribe { swap_id: Option<String> },

    /// Swap event notification
    SwapEvent { event: SwapEvent },
...
pub async fn handle_ws_connection(
    ...
    notification_manager
        .add_client(client_id, sender.clone())
        .await;
    ...
    while let Some(msg) = receiver.recv().await {
        match msg {
            WsMessage::Subscribe { swap_id } => {
                ...
                notification_manager
                    .handle_subscription(&client_id, swap_id_bytes)
                    .await;
            }
            ...
            WsMessage::Unsubscribe { swap_id } => {
                ...
                notification_manager
                    .handle_unsubscription(&client_id, swap_id_bytes)
                    .await;
            }
...
mod tests {
    ...
    async fn test_ws_notification_manager() {
        ...
        let manager = Arc::new(tokio::sync::Mutex::new(WsNotificationManager::new(
            event_rx,
        )));
