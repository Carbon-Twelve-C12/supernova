...
pub mod signature_adapter {
    ...
    pub fn adapt_bitcoin_signature(
        sig: &BitcoinSignature,
    ) -> Result<secp256k1::ecdsa::Signature, HTLCError> {
        ...
        Signature::from_compact(&sig_bytes).map_err(|e| HTLCError::InvalidSignature)
    }
}
...
mod tests {
    ...
    fn test_hash_lock_creation() {
        ...
        let verified = hash_lock
            .verify_preimage(&hash_lock.preimage.unwrap())
            .unwrap();
        assert!(verified);
    }
