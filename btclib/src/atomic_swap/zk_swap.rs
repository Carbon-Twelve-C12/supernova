...
use crate::atomic_swap::{
    AtomicSwapError, SwapSession, SupernovaHTLC,
    ParticipantInfo, HTLCState,
};
...
impl ZKSwapBuilder {
    ...
    pub fn prove_swap_validity(
        ...
        let params = self.params.as_ref()
            .ok_or(ZKSwapError::SetupError("Parameters not initialized".to_string()))?;
    ...
    pub fn verify_swap_validity(
        ...
        let vk = self.vk.as_ref()
            .ok_or(ZKSwapError::SetupError("Verification key not initialized".to_string()))?;
...
pub fn verify_zk_swap_session(
    session: &ZKSwapSession,
) -> Result<bool, ZKSwapError> {
