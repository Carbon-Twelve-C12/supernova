...
impl Drop for RpcTimer {
    fn drop(&mut self) {
        let duration = self.start.elapsed().as_secs_f64();
        RPC_LATENCY.with_label_values(&[&self.method]).observe(duration);
    }
}
...
pub fn record_cache_operation(operation: &str, cache_type: &str) {
    CACHE_OPERATIONS.with_label_values(&[operation, cache_type]).inc();
}
...
pub fn record_websocket_message(direction: &str, message_type: &str) {
    WEBSOCKET_MESSAGES.with_label_values(&[direction, message_type]).inc();
}
...
mod tests {
    ...
    fn test_timer() {
        ...
        let swap_duration = metric_families.iter()
