...
use crate::atomic_swap::{SwapSession, SwapState, error::CacheError};
...
impl AtomicSwapCache {
    ...
    pub async fn get_swap_session(&self, swap_id: &[u8; 32]) -> Option<SwapSession> {
        if let Some(entry) = self.swap_sessions.get(swap_id) {
            if !entry.is_expired() {
                self.stats.swap_hits.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        ...
        self.stats.swap_misses.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        None
    }
    ...
    pub async fn get_bitcoin_tx(&self, txid: &Txid) -> Option<BitcoinTransaction> {
        ...
        if let Some(entry) = cache.get(txid) {
            if !entry.is_expired() {
                self.stats.btc_hits.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                return Some(entry.data.clone());
            } else {
                cache.pop(txid);
                self.stats.evictions.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        ...
        self.stats.btc_misses.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        None
    }
    ...
    pub async fn get_script_result(&self, script_hash: &[u8]) -> Option<bool> {
        ...
        if let Some(entry) = self.script_results.get(&key) {
            if !entry.is_expired() {
                self.stats.script_hits.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        ...
        self.stats.script_misses.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        None
    }
    ...
    async fn evict_expired_swaps(&self) {
        let expired: Vec<_> = self.swap_sessions
            .iter()
    ...
    async fn evict_expired_scripts(&self) {
        let expired: Vec<_> = self.script_results
            .iter()
            .filter(|entry| entry.value().is_expired())
    ...
    pub fn hit_rate(&self) -> f64 {
        let total_hits = self.stats.swap_hits.load(std::sync::atomic::Ordering::Relaxed)
            + self.stats.btc_hits.load(std::sync::atomic::Ordering::Relaxed)
            + self.stats.script_hits.load(std::sync::atomic::Ordering::Relaxed);
            
        let total_misses = self.stats.swap_misses.load(std::sync::atomic::Ordering::Relaxed)
            + self.stats.btc_misses.load(std::sync::atomic::Ordering::Relaxed)
            + self.stats.script_misses.load(std::sync::atomic::Ordering::Relaxed);
...
mod tests {
    ...
    async fn test_swap_session_cache() {
        ...
        assert_eq!(cache.stats.swap_misses.load(std::sync::atomic::Ordering::Relaxed), 1);
        ...
        assert_eq!(cache.stats.swap_hits.load(std::sync::atomic::Ordering::Relaxed), 1);
    }
