...
use crate::crypto::{
    quantum::{ClassicalScheme, QuantumScheme},
    zkp::ZkpType,
};
...
impl supernovaConfig {
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        if self.security.quantum_security_level == 0 || self.security.quantum_security_level > 5 {
            return Err(ConfigError::ValidationError(
                "Quantum security level must be between 1 and 5".to_string(),
            ));
        }

        if self.environmental.treasury_allocation_percentage < 0.0 {
            return Err(ConfigError::ValidationError(
                "Treasury allocation percentage cannot be negative".to_string(),
            ));
        }
...
mod tests {
    ...
    fn test_default_config() {
        ...
        assert_eq!(
            config.crypto.quantum.default_scheme,
    ...
    fn test_serialization() {
        ...
        assert_eq!(
            deserialized.crypto.quantum.enabled,
            config.crypto.quantum.enabled
        );
