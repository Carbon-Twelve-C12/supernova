    ...
    fn calculate_transaction_fees(
        ...
        for tx in block.transactions.iter().skip(1) {
            ...
            for input in tx.inputs() {
                if let Some(utxo) = (context.get_utxo)(&input.prev_tx_hash(), input.prev_output_index()) {
                    input_value = input_value.saturating_add(utxo.amount());
                } else {
                    return Err(UnifiedValidationError::InvalidTransaction(
                        format!("Input references non-existent UTXO: {}:{}", 
            ...
            if input_value < output_value {
                return Err(UnifiedValidationError::InvalidTransaction(
                    format!("Transaction creates value: inputs {} < outputs {}", 
        ...
        for tx in block.transactions.iter().skip(1) {
            for input in tx.inputs() {
                ...
                if let Some(is_mature) = &context.is_coinbase_mature {
                    if !is_mature(&input.prev_tx_hash(), 
                                  input.prev_output_index(), 
                                  maturity_blocks) {
                        return Err(UnifiedValidationError::InvalidTransaction(
                            format!("Spending immature coinbase output: {}:{}", 
                                    hex::encode(input.prev_tx_hash()),
                                    input.prev_output_index())
                        ));
                    }
