...
impl BackupManager {
    ...
    fn load_backup_metadata(&self, path: &Path) -> BackupResult<BackupReference> {
        ...
        let metadata: BackupMetadata = bincode::deserialize(&metadata_bytes)
            .map_err(|e| BackupError::SerializationError(e.to_string()))?;
    ...
    fn prune_old_backups(&self) -> BackupResult<()> {
        ...
        let mut full_backup_count = backup_list
            .iter()
            .filter(|b| b.metadata.backup_type == BackupType::Full)
            .count();
    ...
    pub fn block_to_backup_data(
        ...
        let data = bincode::serialize(&(block, checkpoints))
            .map_err(|e| BackupError::SerializationError(e.to_string()))?;
    ...
    pub fn backup_data_to_block(&self, data: &[u8]) -> BackupResult<(Block, Vec<Checkpoint>)> {
        ...
        // For simplicity, we'll just use bincode
        let (block, checkpoints): (Block, Vec<Checkpoint>) = bincode::deserialize(data)
            .map_err(|e| BackupError::SerializationError(e.to_string()))?;
...
mod tests {
    ...
    fn test_backup_create_and_restore() {
        ...
        let backup_data = backup_manager
            .block_to_backup_data(&block, &[checkpoint.clone()])
            .unwrap();

        // Create backup
        let backup = backup_manager
            .create_backup(1, block.hash(), &[checkpoint], &backup_data)
            .unwrap();
        ...
        let (restored_block, restored_checkpoints) =
            backup_manager.backup_data_to_block(&restored_data).unwrap();
    ...
    fn test_backup_integrity_verification() {
        ...
        let backup = backup_manager
            .create_backup(1, block.hash(), &[], &backup_data)
            .unwrap();
