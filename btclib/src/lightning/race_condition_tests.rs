...
mod tests {
    ...
    fn test_fund_creation_bug_fixed() {
        ...
        println!("Successfully processed {} HTLCs without creating funds", successful_htlcs);
    }
    ...
    fn test_atomic_balance_updates() {
        ...
        let total_updates: usize = handles.into_iter()
            .map(|h| h.join().unwrap())
            .sum();
        ...
        println!("Completed {} balance updates across {} threads", total_updates, num_threads);
        println!("Final balances: local={}, remote={}", final_local, final_remote);
    }
} 