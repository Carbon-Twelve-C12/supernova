...
impl QuantumChannel {
    ...
    pub fn create_commitment_transaction(&self) -> Result<Transaction, ChannelError> {
        ...
        if self.local_balance > 0 {
            let local_script = self.create_quantum_safe_script(&self.local_quantum_keys.public_key)?;
    ...
    pub fn settle_htlc_with_zkp(
        ...
        // Find HTLC
        let htlc_index = self.htlcs.iter().position(|h| h.id == htlc_id)
    ...
    pub fn generate_revocation_secret(&mut self) -> [u8; 32] {
        ...
        self.revocation_secrets.insert(self.commitment_number, secret);
    ...
    pub fn check_quantum_canary(&mut self) -> Result<bool, ChannelError> {
        ...
        let signature = sign_quantum(
            &self.quantum_canary.canary_keys,
