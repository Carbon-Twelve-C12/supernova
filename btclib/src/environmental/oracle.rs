...
impl EnvironmentalOracle {
    ...
    pub fn register_oracle(
        ...
        self.oracles.write().unwrap().insert(oracle_id.clone(), oracle_info);
        self.oracle_metrics.write().unwrap().insert(oracle_id, OracleMetrics::default());
    ...
    pub fn request_verification(
        ...
        let request = VerificationRequest {
            ...
            requester,
            timestamp: Utc::now(),
            expiry: Utc::now() + chrono::Duration::from_std(self.consensus_params.verification_timeout).unwrap(),
            required_specializations,
            bounty,
        };
        
        self.pending_verifications.write().unwrap().insert(request_id.clone(), request);
    ...
    pub fn submit_verification(
        ...
        let oracle_info = oracles.get(&oracle_id)
        ...
        if !self.verify_oracle_signature(&verification_data) {
            return Err(OracleError::InvalidSignature("Invalid oracle signature".to_string()));
        }
    ...
    fn process_consensus(&self, request_id: &str) -> Result<(), OracleError> {
        ...
        let oracle_submissions = submissions.get(request_id)
        ...
        let consensus_reached = consensus_percentage >= self.consensus_params.consensus_threshold as f64;
        
        if !consensus_reached {
            return Err(OracleError::ConsensusNotReached(
                format!("Only {}% agreement", consensus_percentage)
            ));
        }
        ...
        let result = VerificationResult {
            ...
            participating_oracles: oracle_submissions.iter()
        ...
        self.distribute_rewards_and_penalties(&result, &verification_groups, &majority_group.unwrap())?;
    ...
    pub fn verify_rec_certificate(&self, certificate: &RECCertificateInfo) -> Result<VerificationStatus, OracleError> {
        let data = EnvironmentalData::RECCertificate {
            ...
            location: certificate.generation_location.as_ref()
                .map(|r| format!("{}-{}", r.country_code, r.sub_region.as_ref().unwrap_or(&"".to_string())))
                .unwrap_or_default(),
    ...
    pub fn verify_carbon_offset(&self, offset: &CarbonOffsetInfo) -> Result<VerificationStatus, OracleError> {
        let data = EnvironmentalData::CarbonOffset {
            ...
            project_location: offset.project_location.as_ref()
                .map(|r| format!("{}-{}", r.country_code, r.sub_region.as_ref().unwrap_or(&"".to_string())))
                .unwrap_or_default(),
    ...
    fn distribute_rewards_and_penalties(
        ...
        let majority_oracles: HashSet<_> = majority_group.iter()
            .map(|s| s.oracle_id.clone())
            .collect();
            ...
            if !majority_oracles.contains(oracle_id) {
                if let Some(oracle) = oracles.get_mut(oracle_id) {
                    ...
                    if let Some(metric) = metrics.get_mut(oracle_id) {
                        ...
                        metric.total_penalties_paid += self.consensus_params.reward_params.incorrect_penalty;
                    }
                }
            }
        }
    ...
    fn check_cache(&self, request_id: &str) -> Option<CachedVerification> {
        ...
        cache.get(request_id).cloned().filter(|c| c.expires_at > Utc::now())
    }
    ...
    fn cache_verification(&self, request_id: &str, result: VerificationResult) {
        ...
        self.verification_cache.write().unwrap().insert(request_id.to_string(), cached);
    }
    ...
    pub fn get_verification_result(&self, request_id: &str) -> Option<VerificationResult> {
        self.completed_verifications.read().unwrap().get(request_id).cloned()
    }
}
