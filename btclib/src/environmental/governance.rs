...
impl EnvironmentalGovernance {
    ...
    pub fn create_proposal(
        ...
        if !self.config.authorized_voters.contains(&proposer) {
            return Err(GovernanceError::UnauthorizedVote(
                format!("Proposer {} is not authorized", proposer)
            ));
        }
        ...
        match &proposal_type {
            ProposalType::TreasuryAllocation { amount, .. } => {
                if *amount == 0 {
                    return Err(GovernanceError::InvalidProposal("Allocation amount cannot be zero".to_string()));
                }
            ...
            ProposalType::ChangeFeeAllocation { new_percentage } => {
                if *new_percentage < 0.0 || *new_percentage > 100.0 {
                    return Err(GovernanceError::InvalidProposal(
                        format!("Fee allocation percentage must be between 0 and 100, got {}", new_percentage)
                    ));
                }
                ...
                if change > self.config.max_allocation_change {
                    return Err(GovernanceError::InvalidProposal(
                        format!("Allocation change {} exceeds maximum allowed change {}", 
            ...
            ProposalType::FundProject { amount, .. } => {
                if *amount == 0 {
                    return Err(GovernanceError::InvalidProposal("Amount cannot be zero".to_string()));
                }
    ...
    pub fn vote(
        ...
        if !self.config.authorized_voters.contains(&voter) {
            return Err(GovernanceError::UnauthorizedVote(
                format!("Voter {} is not authorized", voter)
            ));
        }
        ...
        if Utc::now() > proposal.voting_ends_at {
            return Err(GovernanceError::VotingPeriodEnded(
                format!("Voting period for proposal {} has ended", proposal_id)
            ));
        }
        ...
        if proposal.status != ProposalStatus::Active {
            return Err(GovernanceError::VotingPeriodEnded(
                format!("Proposal {} is no longer active (status: {:?})", proposal_id, proposal.status)
            ));
        }
    ...
    pub fn finalize_proposal(&mut self, proposal_id: &str) -> Result<ProposalStatus, GovernanceError> {
        ...
        if Utc::now() <= proposal.voting_ends_at {
            return Err(GovernanceError::VotingPeriodEnded(
                format!("Voting period for proposal {} has not yet ended", proposal_id)
            ));
        }
        ...
        let total_possible_votes: u64 = self.config.authorized_voters.iter()
            .map(|voter| self.config.voter_weights.get(voter).copied().unwrap_or(1))
            .sum();
        
        // Check quorum
        let quorum_reached = (total_votes as f64 / total_possible_votes as f64) * 100.0 >= self.config.quorum_percentage;
        
        if !quorum_reached {
            proposal.status = ProposalStatus::Rejected;
            return Err(GovernanceError::QuorumNotReached(
                format!("Quorum not reached for proposal {}: {}% of votes cast (required: {}%)",
                        proposal_id, 
    ...
    pub fn execute_proposal(&mut self, proposal_id: &str) -> Result<(), GovernanceError> {
        ...
        if proposal.status != ProposalStatus::Approved {
            return Err(GovernanceError::InvalidProposal(
                format!("Cannot execute proposal {} with status {:?}", proposal_id, proposal.status)
            ));
        }
        
        // Check time lock
        let time_lock_expired = proposal.voting_ends_at + chrono::Duration::days(self.config.time_lock_days as i64) < Utc::now();
        if !time_lock_expired {
            return Err(GovernanceError::InvalidProposal(
                format!("Time lock for proposal {} has not expired yet", proposal_id)
            ));
        }
        
        // Execute the proposal based on its type
        match &proposal.proposal_type {
            ProposalType::TreasuryAllocation { from_account, to_account, amount } => {
                self.treasury.transfer_between_accounts(*from_account, *to_account, *amount)?;
            },
            ProposalType::ChangeFeeAllocation { new_percentage } => {
        ...
        self.historical_proposals.insert(proposal_id.to_string(), proposal_clone);
    ...
    pub fn cancel_proposal(&mut self, proposal_id: &str, canceller: &str) -> Result<(), GovernanceError> {
        ...
        if proposal.status == ProposalStatus::Executed {
            return Err(GovernanceError::InvalidProposal(
                format!("Cannot cancel executed proposal {}", proposal_id)
            ));
        }
    ...
    pub fn get_proposal(&self, proposal_id: &str) -> Option<&EnvironmentalProposal> {
        self.proposals.get(proposal_id).or_else(|| self.historical_proposals.get(proposal_id))
    }
    ...
    pub fn get_proposals_by_status(&self, status: ProposalStatus) -> Vec<&EnvironmentalProposal> {
        let mut proposals: Vec<&EnvironmentalProposal> = self.proposals.values()
            .filter(|p| p.status == status)
            .collect();
        
        let historical: Vec<&EnvironmentalProposal> = self.historical_proposals.values()
            .filter(|p| p.status == status)
            .collect();
    ...
    fn is_admin(&self, user: &str) -> bool {
        ...
        self.config.authorized_voters.iter().take(3).any(|v| v == user)
    }
    
    /// Process expired proposals
    pub fn process_expired_proposals(&mut self) -> Vec<(String, Result<ProposalStatus, GovernanceError>)> {
        let now = Utc::now();
        
        // Find proposals with ended voting periods
        let expired_ids: Vec<String> = self.proposals.values()
...
mod tests {
    ...
    fn test_proposal_creation() {
        ...
        let result = governance.create_proposal(
            ...
            ProposalType::ChangeFeeAllocation { new_percentage: 3.0 },
            "voter1".to_string(),
            None,
            false,
        );
