...
use crate::script::{ScriptType, identify_script_type, extract_script_hash};
use crate::script::interpreter::{ScriptInterpreter, ScriptError, SignatureChecker};
...
impl<'a> ScriptValidator<'a> {
    ...
    fn validate_p2pkh(
        ...
        let result = interpreter.execute(script_pubkey, &checker)
            .map_err(|e| ScriptVerificationError::ExecutionFailed(format!("{:?}", e)))?;
    ...
    fn validate_p2sh(
        ...
        interpreter.execute(script_sig_without_redeem, &checker)
            .map_err(|e| ScriptVerificationError::ExecutionFailed(format!("{:?}", e)))?;
        
        // Then execute the redeem script
        let result = interpreter.execute(&redeem_script, &checker)
            .map_err(|e| ScriptVerificationError::ExecutionFailed(format!("{:?}", e)))?;
    ...
    fn parse_p2pkh_script_sig(&self, script_sig: &[u8]) -> Result<(Vec<u8>, Vec<u8>), ScriptVerificationError> {
        if script_sig.len() < 2 {
