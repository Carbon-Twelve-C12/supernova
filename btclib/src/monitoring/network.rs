...
impl NetworkMetrics {
    ...
    pub fn new(registry: &Registry, namespace: &str) -> Result<Self, MetricsError> {
        ...
        let peer_connection_time = GaugeVec::new(
            Opts::new(
                ...
                "Peer connection duration in seconds",
            )
            .namespace(namespace.to_string())
            .subsystem("network"),
            &["peer_id"],
        ...
        let network_latency = GaugeVec::new(
            Opts::new(
                "network_latency_ms",
                "Network latency by region in milliseconds",
            )
            .namespace(namespace.to_string())
            .subsystem("network"),
            &["region"],
    ...
    pub fn update_bandwidth_usage(&self, incoming_bytes_per_sec: f64, outgoing_bytes_per_sec: f64) {
        self.bandwidth_usage
            ...
            .set(incoming_bytes_per_sec);
        self.bandwidth_usage
            .with_label_values(&["outgoing"])
            .set(outgoing_bytes_per_sec);
    }
    ...
    pub fn get_summary(&self) -> String {
        ...
        // Bandwidth usage
        summary.push_str(&format!(
            "Bandwidth: {:.2} KB/s incoming, {:.2} KB/s outgoing\n",
