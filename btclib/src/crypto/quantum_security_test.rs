    ...
    fn test_quantum_signature_forgery_prevention() {
        ...
        for (scheme, security_levels) in schemes.iter() {
            ...
            for security_level in security_levels {
                ...
                let valid_result = legitimate_keypair.verify(message, &legitimate_signature).unwrap();
                ...
                let forge_result = legitimate_keypair.verify(message, &attacker_signature).unwrap();
                assert!(!forge_result, "Attacker signature should NOT verify with legitimate public key");
                ...
                let random_result = legitimate_keypair.verify(message, &random_signature).unwrap();
                assert!(!random_result, "Random bytes should NOT verify as valid signature");
                ...
                let modified_result = legitimate_keypair.verify(message, &modified_signature).unwrap();
                ...
                let wrong_msg_result = legitimate_keypair.verify(other_message, &legitimate_signature).unwrap();
                assert!(!wrong_msg_result, "Signature for one message should NOT verify for different message");
                ...
                assert!(!pub_key_forge, "Public key verification should reject forgery");
