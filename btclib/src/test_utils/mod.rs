...
use crate::crypto::signature::{Signature, SignatureType, SignatureVerifier};
...
pub fn mock_signature_verify(_message: &[u8], _signature: &[u8], _public_key: &[u8]) -> bool {
...
impl TestTransactionBuilder {
    ...
    pub fn build(self) -> Result<Transaction, SupernovaError> {
        ...
        if self.inputs.is_empty() {
            return Err(SupernovaError::Transaction(TransactionError::Invalid(
    ...
    pub fn build_and_mine(self) -> Result<Block, SupernovaError> {
        ...
        while !block.header.meets_target() {
            ...
            if block.header.nonce > 1_000_000 {
                ...
                return Err(SupernovaError::Mining(MiningError::NonceExhausted));
            }
        }
...
pub mod fork {
    ...
    pub fn create_fork_scenario(
        ...
        let mut prev_hash = common.last().map(|b| b.hash()).unwrap_or([0u8; 32]);
        ...
        prev_hash = common.last().map(|b| b.hash()).unwrap_or([0u8; 32]);
...
pub mod quantum {
    use super::*;
    use crate::crypto::quantum::{QuantumError, QuantumKeyPair, QuantumParameters, QuantumScheme};
    ...
    impl MockQuantumKeyPair {
        ...
        pub fn mock_sign(scheme: QuantumScheme, security_level: u8, message: &[u8]) -> Vec<u8> {
        ...
        pub fn mock_verify(_public_key: &[u8], _message: &[u8], _signature: &[u8]) -> bool {
            true
        }
    }
    ...
    impl TestQuantumTransactionBuilder {
        ...
        pub fn build_signed_transaction(
            ...
            let _mock_sig =
                MockQuantumKeyPair::mock_sign(self.scheme, self.security_level, &tx.hash());
...
mod tests {
    ...
    fn test_chain_creation() {
        let chain = create_test_chain(5).expect("Failed to create test chain");
