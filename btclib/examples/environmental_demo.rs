...
fn main() -> Result<(), ApiError> {
    ...
    println!(
        "   Current REC settings: priority factor = {:.1}, allocation percentage = {:.1}%",
    ...
    for purchase in &purchases {
        println!(
            "     - {} {} of {:?} for {} millinova (impact score: {:.2})",
            purchase.amount,
    ...
    println!(
        "   New REC settings: priority factor = {:.1}, allocation percentage = {:.1}%",
    ...
    println!(
        "   Transaction emissions: {:.6} kg CO2e",
        tx_emissions.tonnes_co2e * 1000.0
    );
    println!(
        "   Transaction energy usage: {:.6} kWh",
        tx_emissions.energy_kwh
    );
    ...
    println!(
        "   Daily emissions: {:.2} tonnes CO2e",
        daily_emissions.tonnes_co2e
    );
    println!(
        "   Weekly emissions: {:.2} tonnes CO2e",
        weekly_emissions.tonnes_co2e
    );
    ...
    println!(
        ...
        daily_metrics.total_emissions
    );
    println!(
        "   - Energy consumption: {:.2} kWh",
        daily_metrics.energy_consumption
    );
    println!(
        "   - Emissions per transaction: {:.4} kg CO2e",
        daily_metrics.emissions_per_transaction
    );
