...
use btclib::environmental::types::{
    EmissionFactor, EmissionsDataSource, EmissionsFactorType, EnergySource, HardwareType, Region,
};
use chrono::Utc;
...
impl EmissionsCalculator {
    ...
    fn load_default_emission_factors(&mut self) {
        ...
        self.region_factors
            .insert(Region::new("US"), EmissionFactor::new(380.0, 2023));
        self.region_factors
            .insert(Region::new("CA"), EmissionFactor::new(120.0, 2023));
        self.region_factors
            .insert(Region::new("CN"), EmissionFactor::new(630.0, 2023));
        self.region_factors
            .insert(Region::new("EU"), EmissionFactor::new(280.0, 2023));
        self.region_factors
            .insert(Region::new("IN"), EmissionFactor::new(720.0, 2023));
        self.region_factors
            .insert(Region::new("RU"), EmissionFactor::new(500.0, 2023));
        self.region_factors
            .insert(Region::new("AU"), EmissionFactor::new(520.0, 2023));
        self.region_factors
            .insert(Region::new("BR"), EmissionFactor::new(90.0, 2023));
        self.region_factors
            .insert(Region::new("ZA"), EmissionFactor::new(850.0, 2023));

        // Add some sub-regions with more specific factors
        self.region_factors.insert(
            Region::with_sub_region("US", "CA"),
            EmissionFactor::new(210.0, 2023),
        );
        self.region_factors.insert(
            Region::with_sub_region("US", "WA"),
            EmissionFactor::new(90.0, 2023),
        );
        self.region_factors.insert(
            Region::with_sub_region("US", "TX"),
            EmissionFactor::new(410.0, 2023),
        );
        self.region_factors.insert(
            Region::with_sub_region("US", "WY"),
            EmissionFactor::new(790.0, 2023),
        );
    }
    ...
    fn calculate_net_emissions(&self, miner: &MinerInfo, days: u32) -> f64 {
        ...
        let emissions_after_renewables =
            gross_emissions * (1.0 - (miner.renewable_percentage / 100.0));
    ...
    fn get_verification_recommendation(&self, miner: &MinerInfo) -> String {
        ...
        if miner.renewable_percentage > 0.0
            && (miner.rec_certificates.is_none()
                || miner.rec_certificates.as_ref().unwrap().verification_status
...
impl NetworkEmissionsSimulator {
    ...
    fn estimate_network_emissions(&self, days: u32) -> f64 {
        ...
        let covered_emissions: f64 = self
            .miners
            .iter()
...
fn main() {
    ...
    for miner in &[&miner1, &miner2, &miner3] {
        ...
        println!(
            "Hardware: {} (Quantity: {})",
            miner.hardware_type, miner.quantity
        );
        ...
        let treasury_contribution =
            calculator.calculate_treasury_contribution(miner, time_period_days);
        ...
        println!(
            "Recommendations: {}",
            calculator.get_verification_recommendation(miner)
        );
    }
    ...
    println!(
        "Covered Hashrate: {:.2} TH/s ({:.4}% of network)",
        covered_hashrate, covered_percentage
    );
    println!(
        "Estimated Network Energy (30 days): {:.2} GWh",
        network_energy / 1_000_000.0
    );
    println!(
        "Estimated Network Emissions (30 days): {:.2} tonnes CO2e",
        network_emissions
    );
    println!(
        "Estimated Annual Network Emissions: {:.2} million tonnes CO2e",
    ...
    println!(
        "Base Emissions (No Strategy): {:.2} tonnes CO2e",
        base_emissions
    );
    println!(
        "With RECs: {:.2} tonnes CO2e (Reduction: {:.2}%)",
        rec_emissions,
        ((base_emissions - rec_emissions) / base_emissions) * 100.0
    );
    println!(
        "With Offsets: {:.2} tonnes CO2e (Reduction: {:.2}%)",
        offset_emissions,
        ((base_emissions - offset_emissions) / base_emissions) * 100.0
    );
