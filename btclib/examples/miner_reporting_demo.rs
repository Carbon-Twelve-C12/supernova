...
use btclib::environmental::{
    MinerReportingManager, MinerEnvironmentalInfo, VerificationStatus,
    Region, EnergySource, HardwareType, EmissionFactor,
};
...
fn main() {
    ...
    if green_miners.is_empty() {
        ...
        for miner in green_miners {
            println!("- {}: {}% renewable energy", miner.miner_id, miner.renewable_percentage);
        }
    }
    ...
    for miner in manager.list_miners() {
        ...
        println!("- {}: Efficiency {} - {}", miner.miner_id, efficiency, baseline_ratio);
    }
    ...
    println!("- Average renewable energy: {:.1}%", report.average_renewable_percentage);
    println!("- Total hashrate: {:.1} TH/s", report.total_hashrate);
    println!("- Total energy consumption: {:.1} kWh/day", report.total_energy_consumption_kwh_day);
...
fn register_demo_miners(manager: &mut MinerReportingManager) {
    ...
    green_miner.update_performance_metrics(1000.0, 25000.0).unwrap(); // 1,000 TH/s, 25,000 kWh/day
    ...
    green_miner.update_rec_status(true, Some("https://greencerts.example.com/12345".to_string()));
    ...
    // Set performance metrics
    mixed_miner.update_performance_metrics(500.0, 15000.0).unwrap(); // 500 TH/s, 15,000 kWh/day
    ...
    // Add carbon offsets
    mixed_miner.update_offset_status(true, Some("https://carbonoffsets.example.com/67890".to_string()));
    ...
    // Set performance metrics
    fossil_miner.update_performance_metrics(200.0, 18000.0).unwrap(); // 200 TH/s, 18,000 kWh/day (inefficient)
    ...
    // Set performance metrics
    pending_miner.update_performance_metrics(300.0, 5500.0).unwrap(); // 300 TH/s, 5,500 kWh/day (efficient)
