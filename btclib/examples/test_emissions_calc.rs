...
impl EmissionsCalculator {
    fn new() -> Self {
        ...
        calculator.emission_factors.push(EmissionFactor::new("US", 0.38));
        calculator.emission_factors.push(EmissionFactor::new("US-CA", 0.21));
        calculator.emission_factors.push(EmissionFactor::new("US-WA", 0.09));
        calculator.emission_factors.push(EmissionFactor::new("EU", 0.28));
        calculator.emission_factors.push(EmissionFactor::new("CN", 0.63));
        calculator.emission_factors.push(EmissionFactor::new("IN", 0.72));
        calculator.emission_factors.push(EmissionFactor::new("GLOBAL", 0.475));
    ...
    fn calculate_miner_emissions(&self, miner: &MinerInfo) -> (f64, f64, f64) {
        ...
        let daily_energy_kwh = miner.hardware.iter()
...
fn main() {
    ...
    println!("=======================================");
    println!("REC reduction (US miner):   {:.1}%", 100.0 * (1.0 - us_market / us_loc));
    println!("REC reduction (EU miner):   {:.1}%", 100.0 * (1.0 - eu_market / eu_loc.max(0.001)));
    println!("Offset reduction (Asia):    {:.1}%", 100.0 * (1.0 - asia_net / asia_market));
    
    // Show annual energy consumption
    println!("\nAnnual Energy Consumption");
    println!("========================");
    
    let us_energy = us_miner.hardware.iter().map(|hw| hw.daily_energy_consumption_kwh()).sum::<f64>() * 365.0 / 1000.0;
    let eu_energy = eu_miner.hardware.iter().map(|hw| hw.daily_energy_consumption_kwh()).sum::<f64>() * 365.0 / 1000.0;
    let asia_energy = asia_miner.hardware.iter().map(|hw| hw.daily_energy_consumption_kwh()).sum::<f64>() * 365.0 / 1000.0;
