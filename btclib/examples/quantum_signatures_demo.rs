...
fn main() {
    ...
    println!("--------------------------------");
    benchmark_scheme(QuantumScheme::Dilithium, SecurityLevel::Medium as u8, &mut rng);
    ...
    println!("------------------------------------------");
    benchmark_scheme(QuantumScheme::Hybrid(ClassicalScheme::Secp256k1), SecurityLevel::Medium as u8, &mut rng);
    
    println!("\n4. Hybrid Signatures (Ed25519 + Dilithium)");
    println!("----------------------------------------");
    benchmark_scheme(QuantumScheme::Hybrid(ClassicalScheme::Ed25519), SecurityLevel::Medium as u8, &mut rng);
}
...
fn benchmark_scheme(scheme: QuantumScheme, security_level: u8, rng: &mut OsRng) {
    ...
    println!("Invalid message verification time: {:?}", invalid_verify_time);
    println!("Modified message verification result: {} (should be false)", invalid);
