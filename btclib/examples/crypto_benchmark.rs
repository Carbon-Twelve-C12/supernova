...
fn benchmark_falcon(message: &[u8], n_value: usize) {
    ...
    println!("\nFalcon (n = {}, Security Level: {}):", n_value, security_level);
    ...
    // Measure key generation time
    let start = Instant::now();
    let keypair = FalconKeyPair::generate(&mut OsRng, &params)
...
fn demonstrate_verifier(message: &[u8]) {
    ...
    let dilithium_keypair = QuantumKeyPair::generate_dilithium(
        &mut OsRng, 
        QuantumSecurityLevel::Medium
    ).expect("Failed to generate Dilithium keypair");
    ...
    let falcon_params = FalconParameters::with_security_level(5)
        .expect("Failed to create Falcon parameters");
    
    let falcon_keypair = FalconKeyPair::generate(&mut OsRng, &falcon_params)
        .expect("Failed to generate Falcon keypair");
    
    let falcon_sig = falcon_keypair.sign(message)
