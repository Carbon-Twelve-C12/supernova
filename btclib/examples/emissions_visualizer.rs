...
use btclib::environmental::types::{Region, HardwareType};
...
impl MinerSimulation {
    fn new(id: &str, region: Region, hardware_type: HardwareType, unit_count: usize, renewable_percentage: f64) -> Self {
        Self {
            id: id.to_string(),
            region,
            hardware_type,
    ...
    fn simulate_emissions(&mut self, days: usize, volatility: f64) {
        ...
        let base_daily = base_emissions_per_unit * self.unit_count as f64 * regional_factor * renewable_factor;
...
fn draw_comparative_chart(label: &str, data_sets: &[(&str, &[f64])]) {
    ...
    let max_value = data_sets
        ...
        .copied()
        .fold(0.0, f64::max) * 1.1; // Add 10% headroom
...
fn print_miner_summary(miners: &[MinerSimulation]) {
    ...
    for miner in miners {
        ...
        if comparison < 0.0 {
            println!("Performance: {:.1}% better than industry average", -comparison);
        } else {
            println!("Performance: {:.1}% worse than industry average", comparison);
        }
    }
}
...
fn main() {
    ...
    let mut miners = vec![
        MinerSimulation::new(
            "GreenMiner-US",
            Region { country_code: "US".to_string(), sub_region: Some("WA".to_string()) }, 
    ...
    for miner in &mut miners {
        println!("Simulating emissions for {} over {} days...", miner.id, simulation_days);
