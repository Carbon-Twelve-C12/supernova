...
use btclib::atomic_swap::{
    AtomicSwapConfig, AtomicSwapSetup, TimeoutConfig, FeeDistribution, FeePayer,
    SupernovaHTLC, HTLCState, TimeLock, ParticipantInfo,
    CrossChainMonitor, SwapSession, SwapState,
};
use btclib::atomic_swap::crypto::{HashLock, HashFunction, generate_secure_random_32};
...
fn main() {
    ...
    println!("   Timeout: {} seconds from now", time_lock.absolute_timeout - current_time);
    ...
    println!("   Claim with correct secret: {}", if claim_valid { "VALID ✓" } else { "INVALID ✗" });
    
    // Try with wrong secret
    let wrong_secret = [0u8; 32];
    let claim_invalid = htlc.verify_claim(&wrong_secret, &claim_sig).unwrap();
    println!("   Claim with wrong secret: {}", if claim_invalid { "VALID ✓" } else { "INVALID ✗" });
    ...
    let btc_sender_key = bitcoin::PrivateKey::from_slice(&[1u8; 32], bitcoin::Network::Testnet).unwrap();
    let btc_recipient_key = bitcoin::PrivateKey::from_slice(&[2u8; 32], bitcoin::Network::Testnet).unwrap();
    ...
    let btc_address = bitcoin_htlc.create_address(bitcoin::Network::Testnet).unwrap();
    ...
    println!("   Supernova amount: {} base units", session.setup.nova_amount);
