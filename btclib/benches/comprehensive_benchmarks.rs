...
fn bench_crypto(c: &mut Criterion) {
    ...
    group.bench_function("kyber_keygen", |b| {
        b.iter(|| KyberKEM::generate_keypair())
    });
...
fn bench_transactions(c: &mut Criterion) {
    ...
    group.bench_function("tx_hash", |b| {
        b.iter(|| tx.hash())
    });
    ...
    let multi_tx = Transaction {
        version: 1,
        inputs: (0..10).map(|_| TxInput {
            previous_output: Default::default(),
            script_sig: vec![0u8; 72],
            sequence: 0xffffffff,
        }).collect(),
        outputs: (0..10).map(|i| TxOutput {
            value: 5000000 * i,
...
fn bench_mining(c: &mut Criterion) {
    ...
    group.bench_function("pow_validate", |b| {
        b.iter(|| pow.validate(&header))
    });
...
fn bench_lightning(c: &mut Criterion) {
    ...
    group.bench_function("payment_hash_calc", |b| {
        b.iter(|| preimage.payment_hash())
    });
...
fn bench_network(c: &mut Criterion) {
    ...
    // Message validation
    group.bench_function("msg_validate", |b| {
        b.iter(|| msg.validate())
    });
