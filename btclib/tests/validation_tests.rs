...
use btclib::validation::{SecurityLevel, ValidationError, ValidationService};
...
fn test_standard_transaction_validation() {
    ...
    let inputs = vec![TransactionInput::new([1u8; 32], 0, vec![], 0xffffffff)];
    ...
    assert!(
        ...
        "Validation time should be measured"
    );
    assert!(
        result.metrics.transaction_size > 0,
        "Transaction size should be measured"
    );
    assert_eq!(
        result.metrics.verification_ops, 1,
        "Should be one verification operation"
    );
}
...
fn test_quantum_transaction_validation() {
    ...
    let validation_service = ValidationService::new(Config::testnet(), SecurityLevel::Maximum);
    ...
    let mut rng = OsRng;
    let keypair = api
        .generate_quantum_keypair(&mut rng)
        .expect("Failed to generate keypair");

    // Sign the transaction
    let quantum_tx = api
        .sign_transaction_quantum(tx, &keypair)
        .expect("Failed to sign transaction");

    // Validate the quantum transaction
    let result = validation_service
        .validate_quantum_transaction(&quantum_tx, &keypair.public_key)
        .expect("Validation failed");
    ...
    let enhanced_service = ValidationService::new(Config::testnet(), SecurityLevel::Enhanced);

    let enhanced_result = enhanced_service
        .validate_quantum_transaction(&quantum_tx, &keypair.public_key)
        .expect("Validation failed");

    assert!(
        enhanced_result.is_valid,
        "Transaction should be valid with enhanced security"
    );
}
...
fn test_confidential_transaction_validation() {
    ...
    // Create a validation service
    let validation_service = ValidationService::new(Config::testnet(), SecurityLevel::Enhanced);
    ...
    let mut rng = OsRng;
    let (conf_tx, _) = api
        .create_confidential_transaction(inputs, outputs, &mut rng)
        .expect("Failed to create confidential transaction");

    // Validate the confidential transaction
    let result = validation_service
        .validate_confidential_transaction(&conf_tx)
        .expect("Validation failed");
    ...
    assert!(
        result.metrics.verification_ops > 1,
        "Multiple verification operations expected"
    );
}
...
fn test_validation_with_disabled_features() {
    ...
    assert!(
        result.is_err(),
        "Should error when quantum features are disabled"
    );
