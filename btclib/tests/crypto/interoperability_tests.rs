...
fn test_multi_signature_transaction() {
    ...
    let dilithium_keypair = QuantumKeyPair::generate(QuantumScheme::Dilithium, Some(dilithium_params))
        .expect("Failed to generate Dilithium key pair");
    
    let dilithium_signature = dilithium_keypair.sign(tx_data)
    ...
    let falcon_signature = falcon_keypair.sign(tx_data)
    ...
    println!("Multi-signature transaction verification: {:?}", verification_result);
    ...
    assert!(dilithium_result.is_ok(), "Dilithium verification resulted in error");
...
fn test_multi_signature_transaction_failures() {
    ...
    assert!(verification_result.unwrap(), "Transaction should verify with valid Dilithium signature");
    ...
    assert!(tamper_verification.is_ok(), "Tampered verification should not error");
    assert!(!tamper_verification.unwrap(), "Tampered transaction should fail verification");
    
    // Test with a missing public key
    let mut missing_key_tx = TestTransaction::new("tx789", tx_data);
    missing_key_tx.signatures.insert(SignatureType::Dilithium, dilithium_signature);
    // Intentionally not adding the public key
    
    let missing_key_verification = missing_key_tx.verify(&verifier);
    assert!(missing_key_verification.is_err(), "Missing key verification should error");
    if let Err(err) = missing_key_verification {
        assert!(matches!(err, SignatureError::InvalidKey(_)), 
                "Expected InvalidKey error, got: {:?}", err);
    }
}
...
fn test_security_level_compatibility() {
    ...
    for (level, tx_name) in &security_levels {
        ...
        verifier.register(
            SignatureType::Dilithium,
            Box::new(btclib::crypto::signature::DilithiumScheme::new(*level as u8))
        );
        
        // Verify the transaction
        let verification_result = transaction.verify(&verifier);
        assert!(verification_result.is_ok(), "Verification at security level {:?} should not error", level);
        assert!(verification_result.unwrap(), 
                "Transaction should verify with valid Dilithium signature at security level {:?}", level);
        
        println!("Successfully verified transaction at security level: {:?}", level);
    }
}
...
fn test_signature_based_multisig() {
    ...
    let dilithium_keypair2 = QuantumKeyPair::generate(QuantumScheme::Dilithium, Some(dilithium_params))
