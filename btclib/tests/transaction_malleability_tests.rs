...
fn test_basic_transaction_malleability() {
    ...
    assert_ne!(
        original_txid, malleable_txid,
        "Malleated transaction should have a different TXID"
    );
    ...
    assert!(
        validator.validate_transaction_basic(&original_tx),
        "Original transaction should be valid"
    );
    assert!(
        validator.validate_transaction_basic(&malleable_tx),
        "Malleated transaction should be valid"
    );
}
...
fn test_mempool_malleability_handling() {
    ...
    assert!(
        mempool.add_transaction(original_tx.clone()),
        "Original transaction should be accepted"
    );
    ...
    assert!(
        !mempool.add_transaction(malleable_tx.clone()),
    ...
    assert!(
        mempool.add_transaction(malleable_tx),
...
fn test_bip146_protection() {
    ...
    if let Some(high_s_tx) = high_s_tx {
        assert!(
            !validator.validate_transaction_bip146(&high_s_tx),
...
fn test_segwit_malleability_protection() {
    ...
    assert_ne!(
        segwit_tx, malleable_tx,
        "Should be able to create variant transaction"
    );
