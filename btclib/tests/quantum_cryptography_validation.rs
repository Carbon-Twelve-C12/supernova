...
use btclib::crypto::quantum::{
    QuantumKeyPair, QuantumParameters, QuantumScheme, ClassicalScheme,
    QuantumError, verify_quantum_signature
};
...
mod dilithium_validation {
    ...
    fn test_dilithium_all_security_levels() {
        ...
        for (level, level_name) in security_levels {
            ...
            assert_eq!(results.tests_failed, 0, "Dilithium {} security tests failed", level_name);
        }
    }

    fn validate_dilithium_security_level(security_level: u8) -> QuantumTestResults {
        let mut rng = OsRng;
        let params = QuantumParameters::with_security_level(QuantumScheme::Dilithium, security_level);
        ...
        // Verification timing
        let start = Instant::now();
        let verified = keypair.verify(message, &signature).expect("Verification failed");
        ...
        if !keypair.verify(different_message, &signature).unwrap_or(true) {
            tests_passed += 1;
        } else {
            tests_failed += 1;
        }
        ...
        if verify_quantum_signature(&keypair.public_key, message, &signature, params).unwrap_or(false) {
            tests_passed += 1;
        } else {
            tests_failed += 1;
        }
    ...
    fn test_dilithium_quantum_attack_resistance() {
        ...
        assert_ne!(sig1, sig2, "Signatures should be different for different messages");
...
mod sphincs_validation {
    ...
    fn test_sphincs_all_security_levels() {
        ...
        for (level, level_name) in security_levels {
            ...
            print_test_results(&results);
            assert_eq!(results.tests_failed, 0, "SPHINCS+ {} security tests failed", level_name);
        }
    }
...
mod hybrid_validation {
    ...
    fn test_hybrid_schemes() {
        ...
        let hybrid_configs = vec![
            (ClassicalScheme::Secp256k1, "Bitcoin-compatible (secp256k1 + Dilithium)"),
...
mod security_edge_cases {
        ...
        for size in test_sizes {
            ...
            match keypair.sign(&message) {
                Ok(signature) => {
                    ...
                    println!("✓ Message size {} bytes: {}", size, if verified { "PASSED" } else { "FAILED" });
    ...
    fn test_key_serialization_security() {
        ...
        let verified = restored_keypair.verify(message, &signature).expect("Verification failed");
...
fn print_test_results(results: &QuantumTestResults) {
    println!("  Key Generation: {:.2}ms", results.key_generation_time.as_secs_f64() * 1000.0);
    println!("  Signing Time: {:.2}ms", results.signing_time.as_secs_f64() * 1000.0);
    println!("  Verification Time: {:.2}ms", results.verification_time.as_secs_f64() * 1000.0);
