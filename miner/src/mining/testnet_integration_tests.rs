use super::environmental_verification::{EfficiencyAudit, EnvironmentalVerifier, RECCertificate};
...
mod testnet_integration_tests {
    ...
    impl TestnetSimulator {
        ...
        async fn setup_environmental_system(&mut self) {
            ...
            self.environmental_verifier
                .register_trusted_issuer("Green-e".to_string())
                .await;
            self.environmental_verifier
                .register_trusted_issuer("I-REC".to_string())
                .await;
            self.environmental_verifier
                .register_trusted_issuer("TIGR".to_string())
                .await;
        }
        ...
        async fn register_green_miner(
            ...
            self.environmental_verifier
                .register_rec_certificate(cert.clone())
                .await;
            ...
            if let Ok(verified) = result {
                self.miner_profiles
                    .insert(miner_id.to_string(), verified.environmental_profile);
            }
        }
    }
    ...
    async fn test_environmental_certificate_expiry() {
        ...
        assert!(
            ...
            "Should get environmental bonus"
        );
        ...
        assert_eq!(
            expired_reward.environmental_bonus, 0,
            "Expired profile should get no bonus"
        );
    }
    ...
    async fn test_difficulty_adjustment_during_testnet() {
        ...
        assert!(
            new_target >= initial_target / 4,
            "Adjustment should be capped"
        );
    }
    ...
    async fn test_stress_test_many_miners() {
        ...
        for i in 0..100 {
            ...
            sim.register_green_miner(&format!("miner_{}", i), renewable, efficiency)
                .await;
        }
        ...
        assert!(
            avg_reward_per_block < expected_avg * 120 / 100,
    ...
    async fn test_long_term_supply_projection() {
        ...
        for _ in 0..(target_blocks / chunk_size) {
            ...
            let reward =
                calculate_mining_reward(sim.current_height, &EnvironmentalProfile::default());
