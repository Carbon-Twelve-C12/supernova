use super::reward::{
    calculate_base_reward, calculate_environmental_bonus, calculate_mining_reward,
    EnvironmentalProfile,
};
use super::{ENV_BONUS_MAX_TOTAL, HALVING_INTERVAL, MAX_HALVINGS};
...
mod security_tests {
    ...
    fn test_reward_calculation_overflow_protection() {
        ...
        for i in 0..10 {
            ...
            assert!(
                reward2 <= reward1,
                "Reward should decrease or stay same after halving"
            );
            assert!(
                reward1 > 0 || i >= MAX_HALVINGS as u64,
                "Reward should be positive before max halvings"
            );
        }
    }
    ...
    fn test_environmental_bonus_manipulation() {
        ...
        assert_eq!(
            max_bonus, max_expected,
            "Maximum bonus should be capped at 35%"
        );
        ...
        assert!(
            exploit_bonus <= (base_reward as f64 * ENV_BONUS_MAX_TOTAL) as u64,
    ...
    fn test_block_time_manipulation_resistance() {
        ...
        assert_eq!(
            reward1, reward2,
            "Rewards should be deterministic based on height"
        );
    }
    ...
    fn test_environmental_bonus_precision() {
        ...
        for (renewable, efficiency, rec, expected) in test_cases {
            ...
            assert!(
                ...
                "Bonus calculation precision error: got {}, expected {}",
                bonus,
                expected
            );
        }
    ...
    fn test_concurrent_reward_calculation() {
        ...
        for _ in 0..10 {
            ...
            let handle = thread::spawn(move || calculate_mining_reward(height, &profile_clone));
            handles.push(handle);
        }
        ...
        for result in &results {
            assert_eq!(
                result.total_reward, first.total_reward,
    ...
    fn test_reward_after_all_halvings() {
        ...
        assert_eq!(
            mining_reward.total_reward, 0,
            "No rewards should be given after all halvings"
        );
    }
}
