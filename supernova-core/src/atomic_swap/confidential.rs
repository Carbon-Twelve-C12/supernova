...
use crate::atomic_swap::{
    AtomicSwapError, AtomicSwapSetup, HTLCError, HTLCState, ParticipantInfo, SupernovaHTLC,
    SwapSession,
};
...
impl ConfidentialSwapBuilder {
    ...
    pub fn create_shared_commitment(
        ...
        // Decompress points
        let alice_point = alice_commitment
            .decompress()
            .ok_or(ConfidentialError::CommitmentMismatch)?;
        let bob_point = bob_commitment
            .decompress()
            .ok_or(ConfidentialError::CommitmentMismatch)?;
...
mod tests {
    use super::*;
    use crate::atomic_swap::htlc::{FeeStructure, TimeLock};
