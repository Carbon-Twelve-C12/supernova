...
impl BlockchainMetrics {
    ...
    pub fn new() -> Self {
        ...
        let verification_failures = IntCounterVec::new(
            Opts::new(
                "supernova_verification_failures",
                "Verification failures by reason",
            ),
            &["reason"],
        )
    ...
    pub async fn record_block(
        ...
        if let Ok(emissions) = emissions_tracker.calculate_block_emissions(block).await {
            ...
            self.carbon_emissions_per_block
                .observe(emissions.tonnes_co2e * 1000.0); // Convert to kg

            // Record renewable energy percentage
            if let Some(renewable_percentage) = emissions.renewable_percentage {
                self.green_energy_percentage
                    .set(renewable_percentage as i64);
        ...
        if tx_count > 0 {
            self.quantum_signatures_percentage
                .set((quantum_tx_count * 100 / tx_count) as i64);
        }
        ...
        let total_io_count: usize = block
            .transactions
            .iter()
            .map(|tx| tx.inputs().len() + tx.outputs().len())
            .sum();
    ...
    pub fn record_transaction_verification(
        &self,
        tx: &Transaction,
        ...
        self.transaction_verification_time
            .observe(verification_time_ms as f64);
        ...
        if tx.has_quantum_signatures() {
            self.transactions_by_type
                .with_label_values(&["quantum_verified"])
                .inc();
        } else {
            self.transactions_by_type
                .with_label_values(&["classical_verified"])
                .inc();
        }
    }
    ...
    pub fn record_block_propagation(&self, block_hash: &[u8; 32], propagation_time_ms: u64) {
        self.block_propagation_time
            .observe(propagation_time_ms as f64);
    }
    ...
    pub async fn record_transaction_included(&self, tx_hash: &[u8; 32]) {
        ...
        if let Some(time) = first_seen.get(tx_hash) {
            ...
            self.transaction_propagation_time
                .observe(propagation_time as f64);

            // Log if propagation took a long time
            if propagation_time > 5000 {
                warn!(
                    "Transaction {} took {}ms to be included",
                    hex::encode(tx_hash),
                    propagation_time
                );
            }
    ...
    pub async fn get_average_block_interval(&self, block_count: usize) -> Option<f64> {
        ...
        let total_millis = block_times[start_idx..]
            .windows(2)
    ...
    pub async fn generate_report(&self) -> String {
        ...
        if let Some(avg_interval) = self.get_average_block_interval(144).await {
            report.push_str(&format!(
                "Average block interval (24h): {:.2} seconds\n",
                avg_interval
            ));
        }
        ...
        if let Some(next_block) = self.get_estimated_time_to_next_block().await {
            report.push_str(&format!(
                "Estimated time to next block: {:.2} seconds\n",
                next_block
            ));
        }
