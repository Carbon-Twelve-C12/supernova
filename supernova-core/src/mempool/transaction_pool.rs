...
impl TransactionPool {
    ...
    pub fn add_transaction(&self, tx: Transaction) -> Result<(), MempoolError> {
        ...
        if !tx.validate(self.get_utxo_or_mempool()) {
            return Err(MempoolError::InvalidTransaction(
                "Failed basic validation".to_string(),
            ));
        }
        ...
        // Update dependency graph
        {
            let mut graph = self
                ...
                .lock()
                .map_err(|e| MempoolError::Other(format!("Lock poisoned: {}", e)))?;
            let mempool_txs: HashMap<[u8; 32], Transaction> = self
                .transactions
                .iter()
        ...
        // Also clean orphans
        {
            ...
            if orphans.len() > self.config.max_orphan_transactions {
                ...
                orphans.extend(
                    orphans_vec
                        .into_iter()
                        .take(self.config.max_orphan_transactions),
                );
            }
            ...
            let count = self
                ...
                .filter(|entry| {
                    entry.fee_rate >= bucket
                        && (i == buckets.len() - 1
                            || entry.fee_rate < buckets.get(i + 1).copied().unwrap_or(u64::MAX))
                })
                .count();
...

        {
            ...
            orphans_to_process = orphans
                ...
                .filter(|tx| {
                    tx.inputs()
                        .iter()
                        .any(|input| input.prev_tx_hash() == *tx_hash)
                })
                .cloned()
                .collect();
    ...
    fn calculate_min_replacement_fee_rate(&self, conflicts: &[[u8; 32]]) -> u64 {
        ...
        std::cmp::max(base_fee_rate + increment, self.config.min_fee_rate)
    }
        ...
        for desc_hash in to_update {
            ...
            if let Some(package_fee_rate) = graph.calculate_package_fee_rate(
                ...
                &self
                    .transactions
                    .iter()
                    .map(|r| (*r.key(), r.transaction.clone()))
                    .collect(),
...
impl fmt::Debug for TransactionPool {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let orphans_count = self.orphans.read().map(|o| o.len()).unwrap_or(0);
        let size_bytes = self.size_bytes.read().map(|s| *s).unwrap_or(0);
...
mod tests {
    ...
    fn create_test_tx(inputs: Vec<(Vec<u8>, u32)>, output_amount: u64) -> Transaction {
