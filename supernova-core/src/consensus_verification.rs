...
use crate::validation::{SecurityLevel, ValidationError};
...
pub trait VerificationPredicate: Send + Sync {
    ...
    ) -> Result<bool, ConsensusVerificationError>;
...
impl ConsensusVerificationFramework {
    ...
    pub fn verify_block(
        &self,
        block: &Block,
    ) -> Result<VerificationReport, ConsensusVerificationError> {
    ...
    pub fn model_check(
        ...
        depth: usize,
    ) -> Result<ModelCheckingReport, ConsensusVerificationError> {
        ...
        let properties: Vec<_> = self
            ...
            .filter(|p| {
                matches!(
                    p.verification_type,
                    VerificationType::Invariant | VerificationType::Safety
                )
            })
            .collect();
    ...
    pub fn import_specifications(
        &mut self,
        json_spec: &str,
    ) -> Result<(), ConsensusVerificationError> {
        // Parse the JSON specification
        let properties: Vec<ConsensusProperty> = serde_json::from_str(json_spec).map_err(|e| {
            ConsensusVerificationError::SpecificationError(format!("Failed to parse JSON: {}", e))
        })?;
    ...
    pub fn export_specifications(&self) -> Result<String, ConsensusVerificationError> {
        serde_json::to_string_pretty(&self.properties).map_err(|e| {
            ConsensusVerificationError::SpecificationError(format!(
...
impl VerificationReport {
    ...
    pub fn get_critical_failures(&self) -> Vec<&VerificationResult> {
        self.results
            .iter()
            .filter(|r| !r.success && r.is_critical)
            .collect()
    }
}
...
impl InputVerificationPredicate {
    pub fn new() -> Self {
        Self {
            description: "Verifies that all transaction inputs reference existing UTXOs"
                .to_string(),
            is_critical: true,
        }
    }
...
impl VerificationPredicate for TimestampPredicate {
    fn verify_block(
        ...
        if block.header().timestamp > chain_state.current_timestamp + self.max_future_time {
            return Err(ConsensusVerificationError::BlockFromFuture(
        ...
        if block.header().timestamp <= chain_state.current_timestamp / 2 {
            return Err(ConsensusVerificationError::BlockTooOld(
                block.header().timestamp,
            ));
        }
