...
use crate::mining::{BlockTemplate, MiningConfig, MiningWorker};
...
impl MiningManager {
    ...
    pub fn get_mining_info(&self) -> Result<MiningInfo, MiningError> {
        ...
        let environmental_impact =
            self.environmental_tracker
                .as_ref()
                .map(|tracker| EnvironmentalImpact {
    ...
    pub fn get_mining_template(
        ...
        info!(
            "Creating mining template with capabilities: {}",
            capabilities
        );
        ...
        let environmental_data =
            self.environmental_tracker
                .as_ref()
                .map(|tracker| TemplateEnvironmentalData {
    ...
    pub fn submit_block(&self, block_data: &[u8]) -> Result<SubmitBlockResponse, MiningError> {
        ...
        let block: Block = bincode::deserialize(block_data).map_err(|e| {
            MiningError::InvalidTemplate(format!("Failed to deserialize block: {}", e))
        })?;
        ...
        if let Err(e) = self.block_sender.send(block.clone()) {
            return Err(MiningError::SubmissionFailed(format!(
        ...
        if let Some(tracker) = &self.environmental_tracker {
            ...
            info!(
                "Block mining recorded: {} kWh energy, {} g CO2 emissions",
    ...
    pub fn get_mining_stats(&self, period: u64) -> Result<MiningStats, MiningError> {
        ...
        let uptime = if let Some(start_time) = *self
            .start_time
            .read()
            .map_err(|e| MiningError::InternalError(format!("Lock poisoned: {}", e)))?
        {
        ...
        let (carbon_emissions_per_hash, renewable_percentage) =
            if let Some(tracker) = &self.environmental_tracker {
                (
        ...
        Ok(MiningStats {
            ...
            current_difficulty: self
                .target_to_difficulty(self.current_target.load(Ordering::Relaxed) as u32),
    ...
    pub fn get_mining_status(&self) -> Result<MiningStatus, MiningError> {
        ...
        let template_age_seconds = if let Some(created) = *self
            .template_created
            .read()
            .map_err(|e| MiningError::InternalError(format!("Lock poisoned: {}", e)))?
        {
    ...
    pub fn start_mining(&self, threads: Option<u32>) -> Result<(), MiningError> {
        ...
        self.mining_threads
            .store(thread_count as u64, Ordering::Relaxed);
    ...
    pub fn update_mining_config(
    ...
    ) -> Result<MiningConfiguration, MiningError> {
        ...
        if let Some(intensity) = config.intensity {
            if !(0.0..=1.0).contains(&intensity) {
                return Err(MiningError::ConfigError(
    ...
    fn verify_proof_of_work(&self, block: &Block) -> bool {
        ...
        let hash_value =
            u32::from_be_bytes([block_hash[0], block_hash[1], block_hash[2], block_hash[3]]);
    ...
    fn select_transactions_for_block(&self) -> Result<Vec<Transaction>, MiningError> {
        ...
        info!(
            "Selected {} transactions with total fees {} and size {} bytes",
            selected.len(),
            total_fees,
            total_size
        );
    ...
    fn create_coinbase_transaction(
        ...
        let miner_output = TransactionOutput::new(reward, self.miner_address.clone().into_bytes());

        // Create transaction
        let coinbase_tx = Transaction::new(1, vec![coinbase_input], vec![miner_output], 0);
    ...
    fn calculate_block_reward(&self, height: u64) -> u64 {
        ...
        let total_reward =
            base_reward + environmental_bonus + quantum_bonus + early_adopter_bonus + network_bonus;
    ...
    fn calculate_early_adopter_bonus(&self, base_reward: u64, height: u64) -> u64 {
        ...
        if height < EARLY_ADOPTER_PERIOD {
            ...
            let bonus_percentage = (remaining_blocks as f64 / EARLY_ADOPTER_PERIOD as f64)
                * MAX_EARLY_BONUS_PERCENTAGE;
    ...
    fn calculate_merkle_root_for_transactions(
        &self,
    ...
    fn calculate_carbon_intensity_per_hash(&self) -> f64 {
        if let Some(tracker) = &self.environmental_tracker {
            ...
            let network_carbon_intensity = tracker.get_network_carbon_intensity().unwrap_or(475.0); // Default global average

            // Get network hashrate (in TH/s)
            let network_hashrate_ths = tracker.get_network_hashrate().unwrap_or(200_000_000.0); // Approximate current network hashrate
