...
mod tests {
    ...
    use crate::consensus::timestamp_validation::{
        TimestampValidationConfig, TimestampValidator, MAX_FUTURE_TIME,
    };
    ...
    fn test_timestamp_manipulation_at_adjustment_boundary() {
        ...
        let result =
            validator.validate_timestamp(manipulated_timestamp, &timestamps, Some(current_time()));
    ...
    fn test_median_time_past_protection() {
        ...
        let result = validator.validate_timestamp(attack_timestamp, &timestamps, None);
    ...
    fn test_backward_time_travel_attack() {
        ...
        let result = validator.validate_timestamp(backward_timestamp, &timestamps, None);
    ...
    fn test_legitimate_network_time_variance() {
        ...
        for i in 1..timestamps.len() {
            ...
            let result = validator.validate_timestamp(timestamps[i], prior_timestamps, None);

            // Legitimate variance should be accepted
            if timestamps[i] > timestamps[i - 1] {
                assert!(
                    result.is_ok(),
                    "Legitimate timestamp {} rejected",
                    timestamps[i]
                );
            }
        }
    ...
    fn test_recovery_from_time_warp_attempt() {
        ...
        let expected_range =
            (initial_target as f64 * 0.9) as u32..(initial_target as f64 * 1.1) as u32;
