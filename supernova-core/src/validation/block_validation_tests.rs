...
mod tests {
    ...
    use crate::validation::block::{
        BlockValidationConfig, BlockValidationError, BlockValidator, ValidationContext,
    };
    ...
    fn test_full_context_validation() {
        ...
        assert!(matches!(
            result.err().unwrap(),
            BlockValidationError::PrevBlockMismatch
        ));
        ...
        assert!(matches!(
            result.err().unwrap(),
            BlockValidationError::InvalidHeader(_)
        ));
        ...
        assert!(matches!(
            result.err().unwrap(),
            BlockValidationError::TimestampTooEarly(_, _)
        ));
    }
    ...
    fn test_attack_scenario_crafted_header() {
        ...
        assert!(
            result.is_err(),
            "Malicious block should not pass validation!"
        );
