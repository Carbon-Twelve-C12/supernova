...
use crate::crypto::signature::{SignatureParams, SignatureType};
use crate::types::transaction::{SignatureSchemeType, Transaction, TransactionOutput};
use crate::validation::crypto::{CryptoValidationConfig, CryptoValidator};
...
impl TransactionValidator {
    ...
    pub fn validate(&self, tx: &Transaction) -> Result<ValidationResult, ValidationError> {
        ...
        if self.config.require_quantum_resistance {
            if let Some(sig_data) = tx.signature_data() {
                match sig_data.scheme {
                    SignatureSchemeType::Legacy | SignatureSchemeType::Ed25519 => {
                        return Ok(ValidationResult::Invalid(
        ...
        metrics.avg_validation_time_ms =
            ((metrics.avg_validation_time_ms * (metrics.total_validations - 1) as f64) + elapsed)
        ...
        if !tx.is_coinbase() {
            for (i, input) in tx.inputs().iter().enumerate() {
                if input.prev_tx_hash() == [0u8; 32] {
                    return Err(ValidationError::InvalidStructure(format!(
                        "Input {} references null transaction",
                        i
                    )));
                }
            }
    ...
    pub fn validate_signatures(
        ...
        if let Some(sig_data) = tx.signature_data() {
            ...
            match self.crypto_validator.validate_signature(
                ...
                Ok(valid) => {
                    if valid {
                        ...
                        Ok(ValidationResult::Invalid(
                            ValidationError::InvalidSignature(
                                "Extended signature verification failed".to_string(),
                            ),
                        ))
                    }
                }
            ...
            if valid_inputs == tx.inputs().len() {
                ...
                Ok(ValidationResult::Invalid(
                    ValidationError::InvalidSignature(format!(
                        "Only {} of {} input signatures verified",
                        valid_inputs,
                        tx.inputs().len()
                    )),
                ))
    ...
    pub fn validate_output_value(
        ...
        match tx.total_input(&get_output) {
            Some(total_in) => {
                ...
                if fee > total_in / 4 && fee > 1_000_000 {
                    // Only warn if fee > 0.01 NOVA
                    return Ok(ValidationResult::SoftFail(
                        ValidationError::InvalidStructure(format!(
            ...
            None => Ok(ValidationResult::Invalid(
                ValidationError::InvalidStructure(
                    "Could not determine total input value".to_string(),
                ),
            )),
        }
    }
    ...
    pub fn sign_quantum_transaction(
        ...
        match keypair.sign(&msg) {
            ...
            Err(e) => Ok(ValidationResult::Invalid(ValidationError::SignatureError(
                e.to_string(),
            ))),
        }
    }
        ...
        if let Some(sig_data) = transaction.signature_data() {
            ...
            let params = QuantumParameters {
                scheme: match sig_data.scheme {
                    ...
                    SignatureSchemeType::Hybrid => {
                        QuantumScheme::Hybrid(crate::crypto::quantum::ClassicalScheme::Secp256k1)
                    }
                    _ => {
                        return Ok(ValidationResult::Invalid(
                            ValidationError::InvalidSignatureScheme,
                        ))
                    }
                },
