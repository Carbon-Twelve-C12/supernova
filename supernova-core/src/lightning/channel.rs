...
use crate::types::transaction::{
    OutPoint, Transaction, TransactionInput as TxIn, TransactionOutput as TxOut,
};
    ...
    pub fn from_hex(hex_str: &str) -> Result<Self, ChannelError> {
        if hex_str.len() != 64 {
            return Err(ChannelError::InvalidState(
                "Invalid hex length for channel ID".to_string(),
            ));
        }
        ...
        if bytes.len() != 32 {
            return Err(ChannelError::InvalidState(
                "Invalid byte length for channel ID".to_string(),
            ));
        }
...
impl Channel {
        ...
        let commitment_tx = Transaction::new(
            ...
            vec![
                ...
                TxOut::new(
                    ...
                    Script::new_p2wpkh(&self.local_node_id.serialize())
                        .as_bytes()
                        .to_vec(),
                ),
                ...
                TxOut::new(
                    ...
                    Script::new_p2wpkh(&self.remote_node_id.serialize())
                        .as_bytes()
                        .to_vec(),
                ),
            ],
    ...
    pub fn add_htlc(
        ...
        if is_outgoing && self.local_balance_novas < amount_novas {
            return Err(ChannelError::InsufficientFunds(format!(
    ...
    fn create_closing_transaction(&self) -> ChannelResult<Transaction> {
        let funding_outpoint = self.funding_outpoint.as_ref().ok_or_else(|| {
            ChannelError::InvalidState("No funding outpoint available".to_string())
        })?;
    ...
    pub fn force_close(&mut self) -> ChannelResult<Transaction> {
        ...
        let force_close_tx = self.commitment_tx.clone().ok_or_else(|| {
    ...
    pub fn cooperative_close(&self) -> ChannelResult<Transaction> {
        ...
        Ok(Transaction::new(2, vec![input], outputs, 0))
    }
