...
impl PeerDiversityManager {
        ...
        *self.subnet_distribution.entry(subnet).or_insert(0) += 1;
        ...
        if diversity_score < self.min_diversity_score {
            if let Some(peer_to_disconnect) = self.suggest_disconnection() {
                return Err(SecurityError::PeerDiversityTooLow(format!(
                    "Need to disconnect peer {} to improve diversity",
                    peer_to_disconnect
                )));
            }
        }
    ...
    pub fn update_peer_info(
        ...
        let peer = self
            .peers
            .get_mut(peer_id)
            .ok_or_else(|| SecurityError::InternalError(format!("Peer {} not found", peer_id)))?;
    ...
    pub fn suggest_disconnection(&self) -> Option<String> {
        ...
        match &self.connection_strategy {
            ConnectionStrategy::BalanceAcrossSubnets => self.find_most_represented_peer_by_subnet(),
            ConnectionStrategy::GeographicDiversity => self
                .find_most_represented_peer_by_geo()
                .or_else(|| self.find_most_represented_peer_by_subnet()),
            ConnectionStrategy::AsnDiversity => self
                .find_most_represented_peer_by_asn()
                .or_else(|| self.find_most_represented_peer_by_subnet()),
...
impl ConnectionRateLimiter {
    ...
    pub fn is_rate_limited(&self, ip_addr: &str) -> bool {
        ...
        if let Some(times) = self.connection_times.get(ip_addr) {
            ...
            let connections_in_window = times
                .iter()
...
impl SecurityManager {
    ...
    pub fn new(
        ...
        let diversity_manager =
            PeerDiversityManager::new(min_diversity_score, connection_strategy, 3);
        let rate_limiter = ConnectionRateLimiter::new(rate_limit_window);
        let eclipse_prevention =
            EclipsePreventionManager::new(rotation_interval, min_outbound_connections);
    ...
    /// Register an outbound connection
    pub fn register_outbound_connection(
        &self,
        peer_id: String,
        ip_addr: IpAddr,
    ) -> Result<(), SecurityError> {
            ...
            loop {
                ...
                if rotation_needed {
                    ...
                    if !peers_to_rotate.is_empty() {
                        // Log the rotation
                        log::info!(
                            "Rotated {} peers to maintain network diversity",
                            peers_to_rotate.len()
                        );
    ...
    pub fn validate_connection(
        ...
        peer_id: &str,
        ip_addr: IpAddr,
    ) -> Result<ConnectionValidation, SecurityError> {
        // Check rate limits
                        ...
                        Err(_) => match IpSubnet::new(ip_addr, 8) {
                            ...
                            Err(_) => {
                                return Err(SecurityError::InternalError(
                                    "Failed to create IP subnet".to_string(),
                                ))
                            }
                        },
                    },
    ...
    pub fn process_verification_response(
        ...
        if let Ok(mut prevention) = self.eclipse_prevention.write() {
            ...
            if verification_result {
                log::info!(
                    "Peer {} successfully completed identity verification",
                    peer_id
                );
            } else {
        ...
        Err(SecurityError::InternalError(
            "Failed to access eclipse prevention manager".to_string(),
        ))
    }
