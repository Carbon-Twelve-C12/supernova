...
use btclib::types::extended_transaction::{ConfidentialTransaction, QuantumTransaction};
...
fn test_quantum_signature_flow() {
    ...
    let outputs = vec![TransactionOutput::new(90_000_000, vec![])];
    ...
    let verification_result = api
        .verify_quantum_transaction(&quantum_tx, &keypair.public_key)
...
fn test_confidential_transaction_flow() {
    ...
    let mut rng = OsRng;
    let (conf_tx, blinding_factors) = api
    ...
    // Verify the transaction
    let verification_result = api
        .verify_confidential_transaction(&conf_tx)
        .expect("Failed to verify transaction");

    assert!(
        verification_result,
        "Confidential transaction verification failed"
    );
}
...
fn test_transaction_processing() {
    ...
    assert!(
        result.is_ok(),
        "Failed to process quantum transaction: {:?}",
        result
    );
}
...
fn test_confidential_transaction_processing() {
    ...
    assert!(
        result.is_ok(),
        "Failed to process confidential transaction: {:?}",
        result
    );
}
