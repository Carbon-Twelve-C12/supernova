...
fn test_network_signature_verification() {
    ...
    // Create a Dilithium-signed message
    {
        ...
        let signature = keypair.sign(data).expect("Failed to sign with Dilithium");
    ...
    // Create a Falcon-signed message
    {
        ...
        let keypair =
            FalconKeyPair::generate(&mut rng, params).expect("Failed to generate Falcon key pair");

        let data = messages_data[1].as_bytes();
        let signature = keypair.sign(data).expect("Failed to sign with Falcon");
    ...
    for node in &nodes {
        println!(
            "Node {} has {} verified messages",
            node.id,
            node.verified_messages.len()
        );
        assert!(
            !node.verified_messages.is_empty(),
...
fn test_parallel_network_message_processing() {
    ...
    for i in 0..message_count {
        let data = format!(
            "Message {}: Transfer {} BTC to Recipient {}",
            i,
            i as f32 / 10.0,
            i % 5
        )
    ...
    println!(
        "Verification rate: {:.2} messages/second",
        verification_rate
    );
    ...
    for (i, node) in nodes.iter_mut().enumerate() {
        ...
        if i == 0 {
            ...
            assert_eq!(
                node.verified_messages.len(),
                1,
                "Honest node should have 1 verified message"
            );
        } else {
            // Malicious node should not verify the corrupted message
            assert_eq!(
                verified_count, 0,
                "Corrupted message should not be verified"
            );
            assert!(
                node.verified_messages.is_empty(),
                "Malicious node should have 0 verified messages"
            );
        }
    }
