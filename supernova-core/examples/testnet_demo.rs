...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Created high-speed testnet with block time: {} seconds",
        high_speed_config.target_block_time_secs
    );

    // Initialize the testnet
    let mut testnet = TestNetManager::new(high_speed_config);
    println!(
        "Testnet initialized with difficulty: {}\n",
    ...
    match testnet.request_faucet_coins(test_address) {
        Ok(amount) => println!(
            "Successfully distributed {} satoshis to {}",
            amount, test_address
        ),
...
fn simulate_block_mining(
    testnet: &mut TestNetManager,
    count: u64,
    ...
    for i in 0..count {
        ...
        {
            println!(
                "  Block {} - New difficulty: {}",
                i,
                testnet.get_current_difficulty()
            );
        } else {
            println!(
                "  Block {} - Mined at time {} ({}s after previous)",
