...
fn analyze_production_scenarios(monitor: &mut QuantumSignatureMonitor) {
    ...
    println!(
        "  - Total verification time: {:.2}ms",
        block_result.verification_time_ms
    );
    println!(
        "  - Per-transaction time: {:.4}ms",
        block_result.verification_time_ms / 100.0
    );
    println!(
        "  - Throughput: {:.0} tx/sec",
        1000.0 / block_result.verification_time_ms * 100.0
    );
    ...
    println!(
        "  - Channel open time: {:.2}ms",
        ln_result.verification_time_ms
    );
    ...
    println!(
        "  - Batch verification time: {:.2}ms",
        batch_result.verification_time_ms
    );
    println!(
        "  - Aggregated proof size: {:.2}KB",
        batch_result.proof_size_kb
    );
}
...
fn compare_with_labrador_research(results: &HashMap<String, Vec<BenchmarkResults>>) {
    ...
    if let Some(dilithium_results) = results.get("Dilithium") {
        if let Some(result_10k) = dilithium_results
            .iter()
            .find(|r| r.signatures_count == 10000)
        {
            println!("Supernova Dilithium (10k signatures):");
            println!(
                "  - Proving time: {:.2}s",
                result_10k.proving_time_ms / 1000.0
            );
            println!(
                "  - Verification time: {:.2}s",
                result_10k.verification_time_ms / 1000.0
            );
            ...
            if proving_ratio < 1.0 {
                println!(
                    "  ✅ Supernova proving is {:.1}x faster",
                    1.0 / proving_ratio
                );
            } else {
            ...
            if verify_ratio < 1.0 {
                println!(
                    "  ✅ Supernova verification is {:.1}x faster",
                    1.0 / verify_ratio
                );
            } else {
                println!(
                    "  ⚠️  Supernova verification is {:.1}x slower",
                    verify_ratio
                );
            }
