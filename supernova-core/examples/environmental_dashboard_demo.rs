...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  Per Transaction: {:.2} kg CO2e",
        network_data.emissions_per_transaction
    );
    println!(
        "  Per Block: {:.2} kg CO2e",
        network_data.emissions_per_block
    );
    ...
    println!(
        "  {:.1}% of miners use 50%+ renewable energy\n",
    ...
    println!(
        "  Renewable Energy Certificates: ${:.2} ({:.1}%)",
        rec_total,
        (rec_total / total_purchases) * 100.0
    );
    println!(
        "  Carbon Offsets: ${:.2} ({:.1}%)",
        offset_total,
        (offset_total / total_purchases) * 100.0
    );
