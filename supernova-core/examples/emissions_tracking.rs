...
fn simple_tracker_example() -> Result<(), EmissionsError> {
    ...
    println!(
        "Emission Factor: {:.4} kgCO2e/kWh",
        tracker.emission_factor().value
    );
    ...
    println!(
        "Emissions Intensity: {:.6} kgCO2e/TH",
        emissions / (hardware.hash_rate * hours)
    );
...
fn hardware_portfolio_example() -> Result<(), EmissionsError> {
    ...
    println!(
        "Total Devices: {}",
        hardware_portfolio.iter().map(|(_, qty)| qty).sum::<u32>()
    );
    ...
    println!(
        "Emissions per TH: {:.6} kgCO2e/TH",
        total_emissions / (total_hashrate * hours)
    );
...
fn network_emissions_example() -> Result<(), EmissionsError> {
    ...
    let tracker = Arc::new(Mutex::new(EmissionsTracker::new_with_network_config(
        network_config,
    )?));
    ...
    let regional_breakdown = tracker
        .lock()
        .unwrap()
        .calculate_regional_emissions(hours)?;

    println!("Network Configuration:");
    println!(
        "Total Network Hashrate: {:.2} EH/s",
        network_config.total_network_hashrate / 1_000_000.0
    );
    println!(
        "Average Efficiency: {:.2} J/TH",
        network_config.average_network_efficiency
    );

    println!("\nDaily Network Emissions:");
    println!(
        "Total Daily Emissions: {:.2} tonnes CO2e",
        daily_emissions / 1000.0
    );
    println!(
        "Annual Network Emissions: {:.2} million tonnes CO2e",
        (daily_emissions * 365.0) / 1_000_000.0
    );
...
fn renewable_energy_example() -> Result<(), EmissionsError> {
    ...
    println!("Region: {:?}", tracker.region());
    println!(
        "Base Emission Factor: {:.4} kgCO2e/kWh",
        tracker.emission_factor().value
    );
    ...
    println!(
        "Gross Emissions (without renewables): {:.2} tonnes CO2e",
        gross_emissions / 1000.0
    );
    println!(
        "Net Emissions (with renewables): {:.2} tonnes CO2e",
        net_emissions / 1000.0
    );
    println!(
        "Emissions Reduction: {:.2} tonnes CO2e ({:.1}%)",
        (gross_emissions - net_emissions) / 1000.0,
        ((gross_emissions - net_emissions) / gross_emissions) * 100.0
    );
    println!(
        "Effective Emission Factor: {:.4} kgCO2e/kWh",
...
fn verification_example() -> Result<(), EmissionsError> {
    ...
    println!(
        "Effective Renewable Percentage: {:.1}%",
        unverified_tracker.effective_renewable_percentage()
    );
    println!(
        "Effective Emission Factor: {:.4} kgCO2e/kWh",
        unverified_tracker.effective_emission_factor().value
    );
    println!(
        "Total Emissions: {:.2} tonnes CO2e",
        unverified_emissions / 1000.0
    );

    println!("\nEmissions with Verified Renewables:");
    println!(
        "Effective Renewable Percentage: {:.1}%",
        verified_tracker.effective_renewable_percentage()
    );
    println!(
        "Effective Emission Factor: {:.4} kgCO2e/kWh",
        verified_tracker.effective_emission_factor().value
    );
    println!(
        "Total Emissions: {:.2} tonnes CO2e",
        verified_emissions / 1000.0
    );

    println!("\nVerification Impact:");
    println!(
        "Emissions Difference: {:.2} tonnes CO2e",
        (unverified_emissions - verified_emissions) / 1000.0
    );
    println!(
        "Percentage Benefit from Verification: {:.1}%",
    ...
    println!(
        "Monthly Mining Reward: {:.2} BTC (${:.2})",
        monthly_reward, monthly_value
    );
    println!(
        "Environmental Incentive Rate: {:.1}%",
        environmental_incentive_rate * 100.0
    );
    println!(
        "Unverified Renewables Incentive: ${:.2}",
        unverified_incentive
    );
    println!("Verified Renewables Incentive: ${:.2}", verified_incentive);
    println!(
        "Financial Benefit of Verification: ${:.2}",
        verified_incentive - unverified_incentive
    );
