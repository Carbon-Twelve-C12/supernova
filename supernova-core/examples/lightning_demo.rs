...
use btclib::lightning::{
    ...
    LightningConfig, LightningNetwork, LightningNetworkError,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "Opening channel with peer {} with capacity of {} Nova",
        peer_id, channel_capacity
    );
    ...
    let invoice = lightning.create_invoice(invoice_amount, description, expiry)?;
    ...
    info!(
        "Channel closed with transaction: {}",
        hex::encode(&close_tx.hash())
    );
